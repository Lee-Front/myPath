const useTextStyler = () => {
    const partialChangeTextStyle = (blockUuid, style, selection) => {
        ...
        const newHtmlData = makeNewHtml(nodes, style);
    ...
    const getCaretInfoFromNodes = () => {
        ...
        const blockElement = startContainer.parentElement.closest('[name=editable-tag]');
    ...
    const makeNewHtml = (nodes, style) => {
        ...
        const dragInfo = getCaretInfoFromNodes();

        const { splitedNodeDatas, splitedDragInfo } = splitNodes(nodeDatas, dragInfo);
        const styledNodeDatas = applyStyle(splitedNodeDatas, style, splitedDragInfo);

        const { mergedNodeDatas, mergedDragInfo } = mergedNodesWithSameStyle(styledNodeDatas, splitedDragInfo);
    ...
    const splitNodes = (nodeDatas, dragInfo) => {
        ...
        for (let i = 0; i < copyNodeDatas.length; i++) {
            ...
            if (startNodeIndex <= i && i <= endNodeIndex) {
                // 선택된 범위안의 node 들
                const prevText = i === startNodeIndex ? nodeData.textContent.slice(0, startOffset) : '';
                const nextText =
                    i === endNodeIndex ? nodeData.textContent.slice(endOffset, nodeData.textContent.length) : '';
                ...
                if (nextText.length > 0) {
                    nodeData.textContent = nodeData.textContent.slice(0, splitedDragInfo.endOffset);
        ...
        //splitedNodeDatas = splitedNodeDatas.filter((item) => item.textContent !== '');
    ...
    const applyStyle = (splitedNodeDatas, style, splitedDragInfo) => {
        ...
        return splitedNodeDatas.map((nodeData, index) => {
            ...
            if (nodeData.textContent.trim() === '' && nodeData.nodeName === 'SPAN') {
                const preservedStyles = ['link', 'border-bottom', 'text-decoration', 'background'];
            ...
            if (index < splitedDragInfo.startNodeIndex || index > splitedDragInfo.endNodeIndex) {
                return nodeData;
            }
    ...
    const mergedNodesWithSameStyle = (styledNodeDatas, splitedDragInfo) => {
        ...
        const mergedNodeDatas = styledNodeDatas.reduce((acc, cur, index) => {
            ...
            const isSameStyle =
                ...
                prevStyles.every((styleName) => prevData.style[styleName] === curData.style[styleName]);
            ...
            if (prevData && isSameStyle) {
                ...
                } else if (splitedDragInfo.startNodeIndex < index && splitedDragInfo.endNodeIndex > index) {
                    mergedDragInfo.endNodeIndex -= 1;
    ...
    const generateStyledElements = (styledNodeDatas) => {
        return styledNodeDatas.map((node) => {
            ...
            if (styleKeys.length > 0) {
                const newStyle = styleKeys.map((styleName) => styleName + ':' + style[styleName] + ';').join('');
    ...
    const getSelectNodes = (nodes, selection) => {
        ...
        return nodes.filter((_, index) => startNodeIndex <= index && index <= endNodeIndex);
    };
