...
const CardEditor = ({ pathId, readonly }) => {
    ...
    useEffect(() => {
        const scrollMove = () => {
            editorRef.current.scrollBy(0, editorScroll);
            scrollAnimation.current = requestAnimationFrame(scrollMove);
        };
        if (editorScroll) {
            scrollMove();
        }
    ...
    const mouseUp = (e) => {
        eventRef.current.mouseUp(e);
    };
    const mouseMove = throttle((e) => {
        eventRef.current.mouseMove(e);
    }, 50);
    const keyDown = (e) => {
        eventRef.current.keyDown(e);
    };
    ...
    useEffect(() => {
        const newElement = Array.from(contentRef.current.querySelectorAll('[data-uuid]')).filter(
            (item) => item.getAttribute('data-uuid') === newUuid,
        );
    ...
    eventRef.current.mouseDown = (e) => {
        ...
        if (!isFileUploderOpen && !isContextMenuOpen) {
            ...
            const isSelected = editorStore.selectBlocks.find((block) => block.uuid === handleBlockData?.uuid);
            const isHandle = e.target.closest('[name=block-handle]');

            if (isHandle) {
                if (!isSelected) {
                    const block = document.querySelector(`[data-uuid="${handleBlockData.uuid}"]`);
                    const { x, y } = block.getBoundingClientRect();

                    const blocks = findBlocksByPoint(x, y, 'data').filter(
                        (block) => block.uuid === handleBlockData.uuid || block.tagName === 'multiple',
                    );
                ...
                setIsGrabbing(true);
            } else if (e.button !== 2 && !draggable && !isGrabbing) {
                editorStore.setSelectBlocks([]);
            }
        ...
        if (!contextMenu && !filePopup && e.button === 0) {
    ...
    eventRef.current.mouseMove = (e) => {
        ...
        const Contents = Array.from(contentRef.current.querySelectorAll('[data-uuid]'));

        const editorRect = editorRef.current?.getBoundingClientRect();
        if (isGrabbing && (clientY > editorRect.bottom || clientY < editorRect.top)) {
            const delta = clientY - (clientY < editorRect.top ? editorRect.top : editorRect.bottom);
            setEditorScroll(delta);
        } else {
        ...
        const filteredContents = Contents.filter((content) => {
            ...
            if (bottom < editorRect.top || top > editorRect.bottom) {
                return false;
            }
        ...
        if (selectPoint) {
            const distance = Math.sqrt(
                Math.pow(Math.abs(clientX - selectPoint.x), 2) + Math.pow(Math.abs(clientY - selectPoint.y), 2),
            );
    ...
    eventRef.current.mouseUp = (e) => {
        ...
        if (editorStore.selectBlocks.length > 0 && moveMentSideData?.data.uuid) {
            const filteredBlocks = editorStore.selectBlocks.filter((item) => item.tagName !== 'multiple');
    ...
    const findBlocksByPoint = (x, y, type) => {
        let blocks = document.elementsFromPoint(x, y).filter((item) => item.getAttribute('data-uuid'));
    ...
    const getElementByAxis = (elements, pos, axis) => {
        ...
        const nearEl = elements.reduce((prev, curr) => {
            ...
            return Math.abs(prevRect[rectProp] - pos) > Math.abs(currRect[rectProp] - pos) ? curr : prev;
        }, elements[0]);
    ...
    const getElementsByPoint = (elements, x, y) => {
        ...
        const minDistance = nearRect ? Math.min(Math.abs(nearRect?.left - x), Math.abs(nearRect?.right - x)) : null;
        ...
        if (xAxisResults?.hoverEl || (minDistance && minDistance < 25)) {
            ...
            if (blockUuid !== editorStore.handleBlock?.uuid) {
                ...
                editorStore.setHandleBlock({
                    ...
                    position: {
                        x: nearRect.x,
                        y: Math.max(editorTop, nearRect.y),
                    },
                });
            }
        } else {
        ...
        if (!xAxisResults?.nearEl) {
            ...
            nearElement.current = editorStore.findBlock(yAxisResults?.nearEl.getAttribute('data-uuid'));
        } else {
            nearElement.current = editorStore.findBlock(xAxisResults?.nearEl.getAttribute('data-uuid'));
            const hoverBlock = editorStore.findBlock(xAxisResults?.hoverEl?.getAttribute('data-uuid'));
    ...
    const decideMovementSide = (x, y) => {
        ...
        const targetElement = document.querySelector(`[data-uuid="${targetBlock.uuid}"]`);
        ...
        const targetElementData = getMovePositionByBlock(targetBlock, minDistance.position, x);
    ...
    const getNearestSide = (element, x, y) => {
        ...
        if (top <= y && y <= bottom) {
            distanceList.push({
                position: 'left',
                distance: Math.abs(left - x),
            });
            distanceList.push({
                position: 'right',
                distance: Math.abs(right - x),
            });
        }
        ...
        return distanceList.reduce((min, item) => (min.distance > item.distance ? item : min));
    };
    ...
    const getMovePositionByBlock = (targetBlock, position, x) => {
        ...
        if (position === 'top') {
            siblingBlock = hoverBlock ? previousSibling : topParentSiblingsData.previousSibling;
        } else if (position === 'bottom') {
            siblingBlock = hoverBlock ? targetBlock : topParentSiblingsData.nextSibling;
        }

        targetElementData.data = siblingBlock ? siblingBlock : targetBlock;
        targetElementData.position = position === 'top' && siblingBlock ? 'bottom' : position;
        ...
        const isSubTextAreaTag =
            targetElementData.data.tagName === 'checkbox' || targetElementData.data.tagName === 'bullet';

        // 체크박스만 예외적으로 추가처리 필요
        if (isSubTextAreaTag && targetElementData.position === 'bottom') {
            const checkboxElement = contentRef.current.querySelector(`[data-uuid="${targetElementData.data.uuid}"]`);
            const checkboxTextElement = checkboxElement.querySelector(`[name="text-area"]`);
    ...
    const getSiblingsData = (data, blocks) => {
        const siblingElements = blocks.filter((element) => element.parentId === data.parentId);

        const targetIndex = siblingElements.findIndex((element) => element.uuid === data.uuid);
    ...
    const toggleFileUploader = (e) => {
        ...
        if (e.button === 0 && editorStore.hoverBlock?.tagName === 'image') {
            if (!isFileUploderOpen) {
                const hoverElement = document.querySelector(`[data-uuid="${editorStore.hoverBlock.uuid}"]`);
    ...
    const handleEditorClick = (e) => {
        ...
        if (e.button === 0 && e.target === e.currentTarget && !isHandle && !draggable) {
            ...
            if (editorStore.blocks.length > 0) {
                ...
                if (lastBlockData.tagName === 'div' && lastBlockData.html === '') {
                    const lastBloack = document.querySelector(`[data-uuid="${lastBlockData.uuid}"]`);
                    lastBloack.firstChild.focus();
                    return;
                }
            }

            const newElement = editorStore.createBlock({
                pathId,
                tagName: 'div',
            });
            editorStore.saveBlocks([...copyObjectArray(editorStore.blocks), newElement]);
    ...
    const handleEditorContextMenu = (e) => {
        ...
        if (hoverBlock) {
            ...
            if (!contextMenu) {
                ...
                if (!isSelected) {
                    const block = document.querySelector(`[data-uuid="${hoverBlock.uuid}"]`);
                    const { x, y } = block.getBoundingClientRect();

                    const blocks = findBlocksByPoint(x, y, 'data').filter(
                        (block) => block.uuid === hoverBlock.uuid || block.tagName === 'multiple',
                    );
    ...
    return (
        ...
        >
            <ContentWrapper name="content-area" ref={contentRef} onClick={handleEditorClick}>
            ...
            {(isFileUploderOpen || isContextMenuOpen || draggable) && (
                <OverlayContainer zindex={isFileUploderOpen || isContextMenuOpen || draggable}>
