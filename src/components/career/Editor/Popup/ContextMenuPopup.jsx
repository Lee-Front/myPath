...
const ContextMenuPopup = ({ pointer, changeContextMenuYn, popupData }) => {
    ...
    const changeTextStyle = async (blockUuid, style) => {
        const selection = window.getSelection();
    ...
    return (
        <ContextMenuWarpper ref={contextRef} pointer={pointer} className="contextMenu">
            <TextMenuWrapper>
                {/* 폰트사이즈 */}
                <FontSizeSelector
                    uuid={uuid}
                ...
                <TextColorMenu
                    ...
                    handleColorDelete={() => handleStateChange('', setBackground, 'background', '')}
                    changeTextStyle={changeTextStyle}
                >
                    배경
                </TextColorMenu>
                ...
                <TextMenuButton
                    isActive={bold}
                    buttonType={'bold'}
                    onClick={() => handleStateChange(!bold, setBold, 'font-weight', !bold ? 'bold' : '')}
                >
                    B
                </TextMenuButton>
                {/* 기울기 */}
                <TextMenuButton
                    isActive={italic}
                    buttonType={'italic'}
                    onClick={() => handleStateChange(!italic, setItalic, 'font-style', !italic ? 'italic' : '')}
                >
                    i
                </TextMenuButton>
                {/* 밑줄 */}
                <TextMenuButton
                    ...
                    onClick={() =>
                        handleStateChange(!underLine, setUnderLine, 'border-bottom', !underLine ? '0.1rem solid' : '')
                    }
                >
                    U
                </TextMenuButton>
