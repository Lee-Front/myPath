...
const useEditorStore = create((set, get) => ({
    ...
    updateBlocks: (blockUuids, updateData) => {
        let blocks = JSON.parse(JSON.stringify(get().blocks));
        const keys = Object.keys(updateData);
        const excludedBlocks = ['checkbox', 'bullet'];

        blockUuids.forEach((uuid) => {
            const block = blocks.find((block) => block.uuid === uuid);

            if (
                keys.includes('tagName') &&
                !excludedBlocks.includes(updateData['tagName']) &&
                (block.tagName === 'checkbox' || block.tagName === 'bullet')
            ) {
                blocks.forEach((item) => {
    ...
    saveBlocks: async (newBlocks) => {
        ...
        blocks.forEach((element) => {
            ...
            if (sameElement && JSON.stringify(element) !== JSON.stringify(sameElement)) {
    ...
    moveBlocks: (selectDatas, movementData) => {
        ...
        const filteredBlocks = JSON.parse(JSON.stringify(get().blocks)).filter((element) => {
        ...
        if (targetData.parentId) {
            ...
            if (movementData.position === 'top' || movementData.position === 'bottom') {
                if (movementData?.movementSideType === 'text') {
                    ...
                    filteredBlocks.splice(movementData.position === 'top' ? toIndex : toIndex + 1, 0, ...fromDatas);
                } else {
                ...
                const rowChildElements = filteredBlocks.filter((block) => block.parentId === targetData.parentId);
            ...
            if (movementData.position === 'left' || movementData.position === 'right') {
                ...
                if (movementData.position === 'left') {
                    filteredBlocks.splice(toIndex, 0, newElement, newColumElement1, ...fromDatas, newColumElement2);
                } else {
                ...
                fromDatas.forEach(
                    (element) =>
                        (element.parentId = movementData?.movementSideType === 'text' ? findToData.uuid : null),
                );
    ...
    removeColumnAndRowIfEmpty: (blocks) => {
        ...
        if (columns.length > 0) {
            columns.forEach((column) => {
                const columnChildren = newBlocks.filter((block) => block.parentId === column.uuid);
        ...
        if (rows.length > 0) {
            rows.forEach((row) => {
                const rowChildren = newBlocks.filter((block) => block.parentId === row.uuid);
                ...
                if (rowChildren.length <= 1) {
                    ...
                    const rowIndex = newBlocks.findIndex((block) => block.uuid === rowUuid);

                    newBlocks = newBlocks.filter((block) => block.uuid !== rowUuid);
                    newBlocks = newBlocks.filter((block) => block.uuid !== columnUuid);
                    const columns = newBlocks.filter((block) => block.parentId === columnUuid);

                    newBlocks = newBlocks.filter((block) => block.parentId !== columnUuid);
                    columns.forEach((obj) => {
