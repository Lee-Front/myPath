...
const useEditorStore = create((set, get) => ({
  ...
  // blocks 조작 함수들
  createBlock: (pathId, tagName, direction, width) => {
    if (!tagName) {
  ...
  findBlock: (blockUuid) => {
    const blocks = JSON.parse(JSON.stringify(get().blocks));
  ...
  moveElementData: (selectDatas, movementData) => {
    ...
    if (targetData.parentId) {
      ...
      if (
        ...
        const newColumElement1 = editorStore.createBlock({
          pathId,
          tagName: "multiple",
          direction: "column",
          width: 50,
        ...
        const newColumElement2 = editorStore.createBlock({
          pathId,
          tagName: "multiple",
          direction: "column",
          width: 50,
    ...
    if (selectDatas[0].parentId) {
      ...
      if (
        ...
        const remainingElements =
          editorStore.removeColumnAndRowIfEmpty(filteredElements);
