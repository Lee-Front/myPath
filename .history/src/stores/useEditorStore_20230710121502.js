    ...
    if (columns.length > 0) {
      columns.forEach((column) => {
        ...
        if (columnChildren <= 0) {
          ...
          if (get().selectBlocks.find((block) => block.uuid === column.uuid)) {
            const selectBlocks = get().selectBlocks.filter(
              (block) => block.uuid !== column.uuid
            );
            get().setSelectBlocks(selectBlocks);
          }
    ...
    if (rows.length > 0) {
      rows.forEach((row) => {
        ...
        if (rowChildren.length <= 1) {
          ...
          if (get().selectBlocks.find((block) => block.uuid === rowUuid)) {
            const selectBlocks = get().selectBlocks.filter(
              (block) => block.uuid !== rowUuid
            );
            get().setSelectBlocks(selectBlocks);
