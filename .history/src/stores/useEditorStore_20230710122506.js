...
const useEditorStore = create((set, get) => ({
  ...
  moveBlocks: (selectDatas, movementData) => {
    ...
    const remainingElements = get().removeColumnAndRowIfEmpty(filteredBlocks);
    const test = get().selectBlocks.filter((block) => {
      const findBlock = remainingElements.find(
        (element) => element.uuid === block.uuid
      );
  ...
  removeSelectBlocksIfEmpty: (blocks) => {
    const selectBlocks = get().selectBlocks;
    const filteredBlocks = selectBlocks.filter((block) => {
      console.log(get().blocks.find((item) => item.uuid === block.uuid));
    });
