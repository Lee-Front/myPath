...
const useEditorStore = create((set, get) => ({
  ...
  moveBlocks: (selectDatas, movementData) => {
    ...
    if (selectDatas[0].parentId) {
      const fromParentData = get().findBlock(selectDatas[0].parentId);
      if (
        fromParentData.tagName !== "checkbox" &&
        fromParentData.tagName !== "bullet"
      ) {
        const remainingElements =
          get().removeColumnAndRowIfEmpty(filteredBlocks);
        get().saveBlocks(remainingElements);
        return;
      }
    }

    get().saveBlocks(filteredBlocks);
  },
  ...
  toggleSelectBlock: (uuid) => {
    ...
    const isSelected = selectBlocks.includes((blockUuid) => blockUuid === uuid);
    if (isSelected) {
      set((state) => ({
        ...state,
        selectBlocks: selectBlocks.filter((blockUuid) => blockUuid !== uuid),
      }));
    } else {
      set((state) => ({ ...state, selectBlocks: [...selectBlocks, uuid] }));
    }
  },
}));
