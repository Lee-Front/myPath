...
const useTextStyler = () => {
  ...
  const changeTextStyle = (blockUuid, style) => {
    ...
    if (selection.type === "Range") {
      partialChangeTextStyle(blockUuid, style, selection);
    } else {
      fullChangeTextStyle(style);
    }
  };
  ...
  const fullChangeTextStyle = async (style) => {
  ...
  const textStylerFunctions = {
    ...
    fullChangeTextStyle,
    makeNewHtml,getNodesData,splitNodes,applyStyle,mergedNodesWithSameStyle,generateStyledElements,setCaretPosition
  };
  }
