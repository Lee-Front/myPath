...
const ContextMenuPopup = ({
  ...
  const makeNewHtml = (nodes, range, style) => {
    ...
    let newNodeList = []; // nodeList를 돌면서 바뀌는 nodeData를 담을 목록
    ...
    newNodeList = splitNodes(nodeDatas, dragInfo);
    ...
    const currentNodeData = JSON.parse(JSON.stringify(dragInfo));
    ...
    //       dragInfo.startNodeIndex -= 1;
    //       dragInfo.endNodeIndex -= 1;
    //     } else if (index === currentNodeData.startNodeIndex) {
    //       // 현재 순번이 시작 노드 순번이라면
    //       // offset에 이전 text의 길이만큼 추가
    //       dragInfo.startNodeIndex -= 1;
    //       dragInfo.endNodeIndex -= 1;
    //       dragInfo.startOffset += newNodeList[index - 1].textContent.length;
    //       dragInfo.endOffset += newNodeList[index - 1].textContent.length;
    //     } else if (
  ...
  const splitNodes = (nodeDatas, dragInfo) => {
    const newNodeList = [];
    const { startNodeIndex, endNodeIndex, startOffset, endOffset } = JSON.parse(
    ...
    for (let i = 0; i < nodeDatas.length; i++) {
      ...
      if (startNodeIndex <= i && i <= endNodeIndex) {
        ...
        if (prevText.length > 0) {
          ...
          // prevText가 있다는건 분리를 시켜줘야 한다는뜻임
          dragInfo.startNodeIndex += 1;
          dragInfo.startOffset = 0;
          dragInfo.endNodeIndex += 1;

          if (startNodeIndex === endNodeIndex) {
            dragInfo.endOffset -= prevText.length;
          }

          const prevNode = JSON.parse(JSON.stringify(nodeData));
          prevNode.textContent = prevText;
          console.log("prevNode: ", prevNode.textContent);
          newNodeList.push(prevNode);
        }
