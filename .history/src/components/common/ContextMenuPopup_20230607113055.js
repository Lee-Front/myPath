...
const ContextMenuPopup = ({
  ...
  const makeNewHtml = (nodes, range, style) => {
    ...
    console.log("mergedNodeDatas: ", mergedNodeDatas);
    const styledElements = generateStyledElements(mergedNodeDatas);

    mergedDragInfo.html = styledElements;
  ...
  const mergedNodesWithSameStyle = (styledNodeDatas, splitedDragInfo) => {
    ...
    const mergedNodeDatas = styledNodeDatas.reduce((acc, cur, index) => {
      ...
      const prevStyleList = Object.keys(prevData?.style || {});
      const curStyleList = Object.keys(curData.style || {});

      prevStyleList.every((styleName) => {
        if (curStyleList.includes(styleName)) {
          if (styleName === "link") {
            if (prevData.link !== curData.link) {
              return false;
            }
          } else {
            if (prevData.style[styleName] !== curData.style[styleName]) {
              return false;
            }
          }
        }
      ...
      if (
        ...
        } else if (index === splitedDragInfo.startNodeIndex) {
          // 현재 순번이 시작 노드 순번이라면
          // offset에 이전 text의 길이만큼 추가
          mergedDragInfo.startNodeIndex -= 1;
          mergedDragInfo.endNodeIndex -= 1;
          mergedDragInfo.startOffset +=
            styledNodeDatas[index - 1].textContent.length;
          mergedDragInfo.endOffset +=
            styledNodeDatas[index - 1].textContent.length;
        } else if (
  ...
  const generateStyledElements = (mergedNodeDatas) => {
    return mergedNodeDatas.map((node) => {
