...
const ContextMenuPopup = ({
  ...
  const checkStyleIndex = (selection, style) => {
    ...
    if (
      ...
      //const newHtml2 = makeNewHtml(targetNodeList, startOffset, endOffset, style);
  ...
  const makeNewHtml = (node, start, end, style) => {
    ...
    if (node.nodeName === "SPAN") {
      ...
      if (start === 0 && end === textContent.length) {
        // 이 경우에는 span이 온전히 겹치는 경우니까 그냥 style 데이터만 바꾸면됨
      } else {
        // 이 경우에는 span의 부분만 선택된 것이기 때문에 span을 나눠줘야됨
      }
      const styleList = [];
