...
const ContextMenuPopup = ({
  ...
  console.log("popupData?.styleData? : ", popupData?.styleData);
  ...
  const changeFontSize = async (value) => {
    ...
    if (fontSize > 0) {
      ...
      modifyEditDom(popupData.uuid, { styleData: { fontSize } });
  ...
  const changeColor = async () => {
    if (popupData?.styleData?.color !== color) {
      // modifyEditDom(popupData.uuid, { styleData: {color } });
  ...
  return (
    <ContextMenuWarpper
      ...
      onClick={(e) => {
        ...
        const sketchElement = e.target.closest(".sketch");
        // 팔레트가 아니고 팝업이 열려있으면
    ...
    >
      <TextMenuWrapper>
        ...
        <div>
          ...
          {isSketchOpen ? (
            <div className="sketch" style={{ position: "absolute" }}>
              <SketchPicker
                color={color}
                onChange={(e) => {
                  const color = e.hex;
