...
const ContextMenuPopup = ({
  ...
  const makeNewHtml = (nodes, range, style) => {
    ...
    const mergedNodeList = newNodeList.reduce((acc, current, index) => {
      ...
      // 아닐경우에는 배열에 추가
      console.log("a : ", JSON.stringify(acc[acc.length]?.style));
      console.log("b : ", JSON.stringify(current.style));
      if (
        JSON.stringify(acc[acc.length]?.style) === JSON.stringify(current.style)
      ) {
        acc[acc.length].textContent += current.textContent;
      } else {
        acc.push(current);
      }
      return acc;
