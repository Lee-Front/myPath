...
const ContextMenuPopup = ({
  ...
  const makeNewHtml = (nodes, range, style) => {
    ...
    const currentNodeData = JSON.parse(JSON.stringify(splitedDragInfo));

    const mergedNodeList = styledNodeDatas.reduce((acc, current, index) => {
      if (
        ...
        if (index < currentNodeData.startNodeIndex) {
          ...
          dragInfo.startNodeIndex -= 1;
          dragInfo.endNodeIndex -= 1;
          dragInfo.startOffset += styledNodeDatas[index - 1].textContent.length;
          dragInfo.endOffset += styledNodeDatas[index - 1].textContent.length;
        } else if (
