...
const ContextMenuPopup = ({
  ...
  const data = getEditComponentData(popupUuid);
  console.log(data);
  const [uuid, setUuid] = useState(popupUuid);

  const [styleData, setStyleData] = useState({
    fontSize: data?.styleData?.fontSize ? data.styleData.fontSize : 16,
  });
  ...
  useEffect(() => {
    ...
    if (styleData.fontSize > 0) {
      inputRef.current.value = styleData.fontSize;
      modifyEditDom(uuid, { styleData: styleData });
      modifyStyleSave();
    }
  }, [styleData]);
  ...
  const changeFontSize = async (value) => {
    const prevValue = styleData.fontSize;
    let newValue;
    if (value <= 0) {
      newValue = prevValue;
    ...
    setStyleData({ ...styleData, fontSize: newValue });
  };

  const modifyStyleSave = async () => {
    await axios.post("/api/editor/style/save", styleData);
  };
