...
const ContextMenuPopup = ({
  ...
  const mergedNodesWithSameStyle = (styledNodeDatas, splitedDragInfo) => {
    ...
    const mergedNodeDatas = styledNodeDatas.reduce((acc, current, index) => {
      const prevStyle = acc[acc.length - 1]?.style;
      console.log("prevStyle: ", prevStyle);
      console.log("current: ", current);
      if (prevStyle) {
        const isSameStyle = Object.keys(prevStyle).every((key) => {
          return prevStyle[key] === current.style[key];
        });
        if (isSameStyle) {
          // 1. 시작 노드 순번보다 앞인경우
          // 2. 시작 노드
