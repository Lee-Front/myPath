...
const SideBar = ({ isSideBarOpen }) => {
  ...
  // console.log("pathList : ", pathCardStore.pathList);
  // const disassembleSearch = disassemble(searchText);
  // console.log("search : ", disassembleSearch);
  // console.log(
  //   "pathList : ",
  //   pathCardStore.pathList.filter((item) => {
  //     const disassembleTitle = disassemble(item.title);

  //     console.log("disassembleTitle : ", disassembleTitle);

  //   })
  // );

  const words = ["테스트", "강아지", "바보"];
  const filterText = "ㅌ"; // 필터링할 문자
  const filteredList = words.filter((text) => {
    return search(text, filterText) !== -1;
  });
