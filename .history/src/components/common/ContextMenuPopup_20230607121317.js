...
const ContextMenuPopup = ({
  ...
  const mergedNodesWithSameStyle = (styledNodeDatas, splitedDragInfo) => {
    ...
    const mergedNodeDatas = styledNodeDatas.reduce((acc, cur, index) => {
      ...
      let isSameStyle = prevStyles.length === curStyles.length;
      const isSameStyle =
      ...
      if (isSameStyle) {
        isSameStyle = prevStyles.every((styleName) => {
          if (curStyles.includes(styleName)) {
      ...
      if (prevData && isSameStyle) {
        ...
        console.log("2");
        console.log("curData: ", JSON.parse(JSON.stringify(curData)));
        acc.push(curData);
      }
