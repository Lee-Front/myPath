    ...
    mergedNodeList.map((node) => {
      ...
      if (styleKeys.length > 0) {
        ...
        if (node.link.length > 0) {
          newHtml += `<a href="http://${node.link}" style="opacity:0.7; cursor:pointer;color:inherit;font-weight:600;text-decoration:inherit;"><span style="${newStyle}">${node.textContent}</span></a>`;
        } else {
