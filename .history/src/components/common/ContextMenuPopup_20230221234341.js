...
const ContextMenuPopup = ({
  ...
  const makeNewHtml = (nodes, range, style) => {
    ...
    console.log("returnData: ", JSON.parse(JSON.stringify(returnData)));
    ...
    for (let i = 0; i < nodeList.length; i++) {
      ...
      if (startNodeIndex <= i && i <= endNodeIndex) {
        ...
        if (startNodeIndex === endNodeIndex) {
          ...
          console.log("nodeData: ",JSON.parse(JSON.stringify(nodeData));)
    ...
    const mergedNodeList = newNodeList.reduce((acc, current, index) => {
      if (
        ...
        console.log("startNodeIndex : ", currentNodeData.startNodeIndex);
        console.log("startNodeIndex : ", currentNodeData.endNodeIndex);
        // 1. 시작 노드 순번보다 앞인경우
        // 2. 시작 노드
        ...
        if (index < currentNodeData.startNodeIndex) {
          ...
          index < currentNodeData.startNodeIndex &&
