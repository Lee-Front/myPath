...
const ContextMenuPopup = ({
  ...
  useEffect(() => {
    ...
    styleDataSave();
  }, [uuid, fontSize, color, backgroundColor]);
  ...
  const changeBackgroundColor = async () => {
    if (popupData?.styleData?.backgroundColor !== color) {
      modifyEditDom(popupData.uuid, {
        styleData: { ...popupData?.styleData, backgroundColor },
      });
    }
  };
  ...
  return (
    <ContextMenuWarpper
      ...
      onClick={(e) => {
        ...
        const backgroundSketchElement = e.target.closest(
      ...
      <TextMenuWrapper>
        ...
        <div>
          ...
          {isBackgroundSketchOpen ? (
            <div
              className="background-color-sketch"
              style={{ position: "absolute", marginTop: "0.2rem" }}
            >
              <SketchPicker
