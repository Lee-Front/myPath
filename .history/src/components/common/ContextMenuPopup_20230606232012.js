...
const ContextMenuPopup = ({
  ...
  const makeNewHtml = (nodes, range, style) => {
    ...
    console.log("endOffset: ", { startOffset, endOffset });
    ...
    for (let i = 0; i < nodeList.length; i++) {
      ...
      if (startNodeIndex <= i && i <= endNodeIndex) {
        ...
        // i가 index와 같은 경우에만 해주면 되지 않을까?
        // 사이에 껴있는건 어차피 일괄로 넣으니까
        const prevText =
          i === startNodeIndex ? textContent.slice(0, startOffset) : null;
        const nextText =
          i === endNodeIndex
        ...
        if (startNodeIndex === endNodeIndex) {
          ...
          if (prevText.length > 0) {
            ...
            returnData.startOffset = 0;
            returnData.endNodeIndex += 1;
            console.log("off : ", JSON.parse(JSON.stringify(returnData)));
            returnData.endOffset -= prevText.length;
            console.log("prevText.length: ", prevText.length);
            // 0,4    1,1   => 1,0  2,1
            let splitNode = JSON.parse(JSON.stringify(nodeData));
            splitNode.textContent = prevText;
            newNodeList.push(splitNode);
