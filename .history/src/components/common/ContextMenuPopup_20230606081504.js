...
const ContextMenuPopup = ({
  ...
  useEffect(() => {
    ...
    if (selection.type === "Range") {
      selectNodes = findSelectNodes(nodes, selection);
    }
    console.log("selectNodes: ", selectNodes);
    const nodesStyle = getNodesStyle(selectNodes);
    console.log("nodesStyle: ", nodesStyle);
    const commonStyles = findCommonAttributes(nodesStyle);
  ...
  const findCommonAttributes = (array) => {
  ...
  const findSelectNodes = (nodes, selection) => {
  ...
  const makeNewHtml = (nodes, range, style) => {
    ...
    console.log({ startNodeIndex, endNodeIndex });
      ...
      if (styleKeys.length > 0) {
        ...
        if (node.link) {
          ...
          linkTag.style.cssText =
            "opacity: 0.7; cursor: pointer; color: inherit; font-weight: 500; text-decoration: inherit;";
