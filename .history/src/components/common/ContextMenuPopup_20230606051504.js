...
const ContextMenuPopup = ({
  ...
  useEffect(() => {
    ...
    const styleArray = selectNodes.map((node) => {
    ...
    const attributes = Object.keys(styleArray[0]);

    const commonStyles = attributes.filter((attribute) => {
      return styleArray.every((obj) => obj.hasOwnProperty(attribute));
    });
    console.log("commonStyles: ", commonStyles);
    console.log(attributes.includes("font-weight"));
    console.log("aa ", !!commonStyles["font-weight"]);
