...
const ContextMenuPopup = ({
  ...
  const makeNewHtml = (nodes, range, style) => {
    ...
    console.log(
      "styledNodeDatas : ",
      JSON.parse(JSON.stringify(styledNodeDatas))
    );
    ...
    console.log(
      "mergedNodeDatas : ",
      JSON.parse(JSON.stringify(mergedNodeDatas))
    );
  ...
  const generateStyledElements = (styledNodeDatas) => {
    return styledNodeDatas.map((node) => {
      Object.keys(node.style).forEach((nodeStyle) => {
        if (nodeStyle === "link") {
          node.link = node.style[nodeStyle];
          delete node.style["link"];
        }
      });
      ...
      if (styleKeys.length > 0) {
        const newStyle = styleKeys
          .map((styleName) => styleName + ":" + node.style[styleName] + ";")
          .join("");

        newElement = document.createElement("span");
        newElement.style.cssText = newStyle;
        newElement.innerText = node.textContent;
      } else {
        newElement = document.createTextNode(node.textContent);
      }
