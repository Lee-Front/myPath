...
const ContextMenuPopup = ({
  ...
  const makeNewHtml = (nodes, range, style) => {
    ...
    const mergedNodeList = newNodeList.reduce((acc, current, index) => {
      // 현재 data와 다음 node의 style data가 일치하면 현재 데이터에 text만 추가해줌
      // 아닐경우에는 배열에 추가
      if (
        JSON.stringify(acc[index + 1]?.style) === JSON.stringify(current.style)
      ) {
        // 이미 존재하는 객체의 style 속성에 현재 객체의 style 속성 추가
        current.textContent += acc[index + 1].textContent;
      } else {
        // 존재하지 않는 경우 현재 객체를 acc 배열에 추가
        acc.push(current);
      }
      return acc;
    }, []);

    console.log("mergedNodeList: ", mergedNodeList);
    mergedNodeList.map((node) => {
      ...
      if (node.style.length > 0) {
        ...
        for (let i = 0; i < styleKeys.length; i++) {
          const styleName = styleKeys[i];
          newStyle += `${styleName}:${node.style[styleName]}`;
        }
