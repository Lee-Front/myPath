...
const ContextMenuPopup = ({
  ...
  const checkStyleIndex = (anchorOffset, focusOffset) => {
    console.log({ anchorOffset, focusOffset });
    if (anchorOffset !== focusOffset) {
      console.log("부분 : ", popupData.html);
      const pre = popupData.html.slice(0, anchorOffset);
      const next = popupData.html.slice(focusOffset, popupData.html.length);
    } else {
      console.log("전체");
    }
  };
  ...
  const changeColor = async (modifyColor) => {
    ...
    checkStyleIndex(anchorOffset, focusOffset);
