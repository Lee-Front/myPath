  ...
  const makeNewHtml = (nodes, range, style) => {
    ...
    for (let i = 0; i < nodeList.length; i++) {
      ...
      if (startNodeIndex <= i && i <= endNodeIndex) {
        ...
        const prevText =
          i === startNodeIndex ? textContent.slice(0, startOffset).trim() : "";
        const nextText =
          i === endNodeIndex
            ? textContent.slice(endOffset, textContent.length).trim()
            : "";
        ...
        const betweenNode = JSON.parse(JSON.stringify(nodeData));
        betweenNode.textContent = nodeData.textContent.slice(startOffset, endOffset));
        newNodeList.push(betweenNode);
        ...
        console.log("startOffset : ", JSON.parse(JSON.stringify(startOffset)));
        console.log("endOffset : ", JSON.parse(JSON.stringify(endOffset)));
