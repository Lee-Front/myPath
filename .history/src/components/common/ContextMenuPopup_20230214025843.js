...
const ContextMenuPopup = ({
  ...
  const checkStyleIndex = (anchorOffset, focusOffset, style) => {
    if (anchorOffset !== focusOffset) {
      ...
      testSelectPosition(editableTag, start, end);
    } else {
      console.log("전체");
    }
  };

  const testSelectPosition = (target, start, end) => {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (currentNodeStartIndex <= start && currentNodeEndIndex > start) {
        console.log("after start : ", start - currentNodeStartIndex);
        // 셀랙트 시작점이 해당 node의 범위안에 있는경우
        console.log("nodes[i]: ", nodes[i]);
        newRange.setStart(nodes[i], start - currentNodeStartIndex);
      }
      ...
      if (currentNodeStartIndex < end && currentNodeEndIndex >= end) {
        ...
        console.log(
          "after end : ",
          textLength - (currentNodeStartIndex - start)
        );
        console.log("nodes[i]: ", nodes[i]);
        newRange.setEnd(
          nodes[i],
          textLength - 1 - (currentNodeStartIndex - start)
        );
      }
      currentNodeStartIndex += textLength + 1;
    }
