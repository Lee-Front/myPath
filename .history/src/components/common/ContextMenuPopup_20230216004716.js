...
const ContextMenuPopup = ({
  ...
  const checkStyleIndex = (selection, style) => {
    ...
    if (
      ...
      if (targetNodeList.length > 1) {
        // 1개 이상인 경우는 span이 존재하기 때문에 분리 및 새로운 html을 만들어줘야한다
        targetNodeList.map((node, index) => {
          // 스타일 없는 node
          if (node.nodeName === "#text") {
            console.log("1");
            // 스타일 먹인 span으로 만들어주면됨
            if (index === 0) {
              // 첫번째 node는
            } else {
            }
          } else {
            //기존 style에 해당 스타일을 더하거나 없애면됨
            if (index === 0) {
              const newHtml2 = makeNewHtml(node, startOffset, endOffset, style);
              console.log("newHtml2 : ", newHtml2);
              // 첫번째 node는
            } else {
            }
          }
  ...
  const makeNewHtml = (node, anchorOffset, focusOffset, style) => {
    ...
    if (node.nodeName === "SPAN") {
      ...
      console.log(node.style.length);
      const styleList = [];
      for (let i = 0; i < node.style.length; i++) {
        const styleName = node.style[i];
        console.log("styleName : ", styleName);
        console.log("value : ", node.style[styleName]);
      ...
      next =
        anchorOffset < focusOffset
          ? textContent.slice(focusOffset, textContent.length)
          : textContent.slice(anchorOffset, textContent.length);

      html =
        anchorOffset < focusOffset
          ? textContent.slice(anchorOffset, focusOffset)
          : textContent.slice(focusOffset, anchorOffset);
    }
