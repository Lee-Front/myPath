...
const CardEditor = () => {
  ...
  const TestClone = ({ children, anotherProps }) => {
    const parentsVar = "Tired";
    return (
      <div>{React.cloneElement(children, { parentsVar, anotherProps })}</div>
    );
  };
  ...
  return (
    <EditorWrapper
      onPointerDownCapture={() => {
        setSelectElement(nearElement);
      }}
      onPointerUpCapture={() => {
        setSelectElement(null);
      }}
    >
      <TestClone anotherProps="직접 내려주기"></TestClone>
      <CardTitle>
        <EditComponent
          tagName="div"
          html=""
