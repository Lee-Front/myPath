...
const PathList = ({ cardType }) => {
  let defaultCount = Math.floor(window.innerWidth / 320);
  defaultCount = defaultCount ? defaultCount : 1;
  const [column, setColumn] = useState(defaultCount);

  const changePathColumn = () => {
    const columnCount = Math.floor(window.innerWidth / 320);
