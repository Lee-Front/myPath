...
const PathList = () => {
  ...
  const getMaxCardCount = () => {
    ...
    console.log("window.innerWidth : ", calc(window.innerWidth - "4rem"));
  ...
  const [column, setColumn] = useState(getMaxCardCount);
  ...
  useEffect(() => {
    window.addEventListener("resize", () => setColumn(getMaxCardCount));
  }, []);
