...
const PathList = () => {
  ...
  const getPathList = async (id) => {
    const userId = qs.stringify(id);
    const pathList = await axios.get("/api/getPathList", { params: userId });
  ...
  const changePathColumn = () => {
    ...
    columnCount = Math.floor(CardListWidth / 240);

    if (CardListWidth < columnCount * 240) {
      columnCount--;
    }
