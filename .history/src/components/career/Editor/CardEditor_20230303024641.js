...
const CardEditor = ({ pathId }) => {
  ...
  const windowMouseMove = (e) => {
    ...
    currentPoint.current = { x: clientX, y: clientY };
      ...
      >
        <OverlayContentWrapper>
          <OverlayContentContents>
            ...
            ></div>
            {selectElements.current.length > 0 && (
              <div
                style={{
                  position: "absolute",
                  width: "100%",
                  left: currentPoint.current?.x + "px",
                  top: currentPoint.current?.y - 10 + "px",
                  opacity: "0.4",
                }}
              >
                {selectElements.current?.map((element) => {
