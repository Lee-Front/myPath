...
const CardEditor = () => {
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    if (targetElement && selectElements.length > 0) {
      ...
      // ) {
      //   equalData = parentData;
      // } else {
      //   equalData = targetData;
      // }

      // equalParentElements = editDom.filter((element) => {
      //   return element.parentId === equalData.parentId;
      // });

      // const targetIndex = equalParentElements.findIndex(
      //   (element) => element.uuid === equalData.uuid
      // );
        ...
        if (hoverElement) {
          if (minDistance.position === "top") {
            targetElementData.uuid =
              //prevData && prevData?.uuid !== selectElements[0].uuid
              prevData ? prevData.uuid : equalData.uuid;

            targetElementData.position =
              //prevData && prevData?.uuid !== selectElements[0].uuid
              prevData ? "bottom" : minDistance.position;
          } else {
            targetElementData.uuid = equalData.uuid;
            targetElementData.position = minDistance.position;
          }
        } else {
          // nearElement니까 최대한 밖에있는 Element여야됨
          console.log("targetData : ", targetData);
          console.log("getTopParentData : ", getTopParentData(targetData));
          console.log("!");
        }
      ...
      if (parentData) {
        if (parentData.direction === "column") {
          ...
          //console.log("position : ", minDistance.position);
          if (
            ...
            if (
              (minDistance.position === "top" && !siblingData.prevData) ||
              (minDistance.position === "bottom" && !siblingData.nextData)
            ) {
              console.log("parentData : ", parentData);
              console.log("parantId: ", parentData.parantId);
              //targetElementData.uuid = parentData.parantId;
            }
          }
        }
      }
  ...
  const getTopParentData = (data) => {
    if (data.parentId) {
      console.log("parent")
      getTopParentData(data.parentId);
      console.log("getTopParentData(data.parentId); : ",getTopParentData(data.parentId);)
    } else {
