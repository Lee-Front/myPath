...
const CardEditor = ({ pathId }) => {
  ...
  const moveElementData = (selectDatas, movementData) => {
    ...
    const filteredElements = copyEditDom().filter((element) => {
  ...
  const removeColumnAndRowIfEmpty = (elementList, uuid) => {
    const copyElementList = elementList.slice();
    copyElementList[0].test = "test";

    console.log("elementList: ", elementList);
    console.log("copyElementList: ", copyElementList);
