...
const CardEditor = ({ pathId }) => {
  ...
  const removeColumnAndRowIfEmpty = (elementList) => {
    let copyElements = copyObjectArray(elementList);

    const columnElements = filterByKey(copyElements, "direction", "column");
    if (columnElements.length) {
      columnElements.forEach((element) => {
        ...
        if (!element.parentId) {
          copyElements = filterByKey(copyElements, "uuid", element.uuid);
          copyElements.filter((obj) => obj.uuid !== element.uuid);
        }
    ...
    const rowElements = filterByKey(copyElements, "direction", "row");
    ...
    if (rowElements.length) {
      rowElements.forEach((element) => {
        ...
        if (rowChildElements <= 1) {
          const rowUuid = element.uuid;

          console.log("1 : ", ...copyElements);
          copyElements.forEach((obj) => {
            if (obj.parentId === rowUuid) {
              obj.parentId = null;
            }
          });
          // 해당 row와 row의 하위 column 삭제
          copyElements = copyElements.filter(
            (obj) =>
              obj.uuid !== element.uuid ||
              (obj.uuid !== element.uuid && obj.parentId !== rowUuid)
          );
        }
      });
    }
