...
const CardEditor = ({ pathId }) => {
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    const targetElement = document.querySelector(`[data-uuid="${targetUuid}"]`);
    const hoverBlock = editorStore.hoverBlock;
    ...
    if (isSubTextAreaTag && targetElementData.position === "bottom") {
      ...
      const { x, y, left, right } = checkboxTextElement.getBoundingClientRect();
      const test = findElementsByPoint(x, y);
