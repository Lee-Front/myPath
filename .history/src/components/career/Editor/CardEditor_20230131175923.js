...
const CardEditor = ({ pathId }) => {
  ...
  const moveElementData = (from, to) => {
    ...
    if (from[0].parentId) {
      ...
      console.log("fromParentData : ", fromParentData);
      if (fromParentData.tagName === "multiple") {
        const { prevData, nextData } = getSiblingsData(fromData);
        console.log("prevData : ", prevData);
        console.log("nextData : ", nextData);
      } else if (fromParentData.tagName !== "checkbox") {
