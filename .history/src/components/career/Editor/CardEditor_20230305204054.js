...
const CardEditor = ({ pathId }) => {
  ...
  const modifyDomSave = async (newEditDom) => {
    ...
    const createList = newEditDom.filter((x) => {
      if (!editDomList.some((y) => x.uuid === y.uuid)) {
        const test = { type: "create", data: x };
        return test;
      }
    });

    // 기존 dom에는 있으나 newDom에는 없는것 [삭제]
    const deleteList = editDomList.filter((x) => {
      if (!newEditDom.some((y) => x.uuid === y.uuid)) {
        const test = { type: "create", data: x };
        return test;
      }
    });
    ...
    // 1. original에 있으나 newEdit에는 없는것 [삭제]
    // 2. 양쪽 다 있으나 값이 다른것 [수정]
    // 3. original에 없으며 newEdit에 있는것 [생성]
  ...
  const getDifferentType = (originalElement, newEditDom) => {
    ...
    if (!matchCount) {
      ...
      if (newEditDom.some((obj) => originalElement.uuid === obj.uuid)) {
        console.log("delete");
        differentType.type = "delete";
      } else {
        console.log("create");
        differentType.type = "create";
      }
    }
      ...
      <CardEditorContentWrapper
        ...
        onMouseUp={(e) => {
          if (
            ...
            const test = getDifferentType(editDom, [...editDom, newElement]);
