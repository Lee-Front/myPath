  ...
  const getDifferentType = (originalElement, newEditDom) => {
    ...
    newEditDom.map((element) => {
      if (originalElement.uuid === element.uuid) {
        ...
        const newKeys = newEditKeys.filter((key) => !elementKeys.includes(key));

        newEditKeys.map((key) => {
          console.log("key : ", key);
          console.log("originalElement[key] : ", originalElement[key]);
          console.log("elementKeys.includes(key): ",elementKeys.includes(key))
          if (elementKeys.includes(key)) {
            // 오리지날에 키가 없음
          } else if (originalElement[key] !== element[key]) {
