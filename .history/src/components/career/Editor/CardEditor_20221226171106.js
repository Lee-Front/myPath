  ...
  const decideMovementSide = (x1, y1) => {
    ...
    if (targetElement && selectElements.length > 0) {
      ...
      if (minDistance.position === "top" || minDistance.position === "bottom") {
        ...
        console.log("includeElements : ", includeElements);

        if (minDistance.position === "bottom") {
          targetData = nextData ? nextData : targetData;
        } else if (minDistance.position === "top") {
          // prevData가 없는 경우는... 최상단 혹은 multiple내에서 최상단.
          // 일반적인 최상단인 경우에는 nextData의 bottom으로
          // multiple의 최상단인 경우에는 row의 top으로 바꿔줘야됨
          if (parentData) {
            console.log("multiple임");

            // prevData가 없으면 최상위걸 가져와서 처리
            // 있으면 내부에서 해야지

            if (prevData) {
              console.log("아직 상단에 있음");
            } else {
              console.log("없음");
            }
            // 일단 최상단 data를 가져와야함
            const topParentdata = getTopParentData(targetData);
            const parentSiblingsData = getSiblingsData(topParentdata);

            console.log("parentSiblingsData : ", parentSiblingsData);
            targetElementData.uuid = parentSiblingsData.prevData
              ? parentSiblingsData.prevData.uuid
              : topParentdata.uuid;
            targetElementData.position = parentSiblingsData.prevData
              ? "bottom"
              : minDistance.position;
            //
            // targetElementData.uuid = prevData ? prevData.uuid : nextData.uuid;
            // targetElementData.position = minDistance.position;
          } else {
            // prevData가 있으면 prevData의 bottm 없으면
        ...
        //   if (minDistance.position === "bottom") {
        //     targetData = includeElements && prevData ? prevData : targetData;
        //   } else if (minDistance.position === "top") {
        //     targetData = includeElements && nextData ? nextData : targetData;
        //   }
      ...
      if (parentData) {
        ...
        // }
        // 일단 여기까지---
        // if (minDistance.position === "left") {
        //   if (parentSiblingData.prevData) {
        //     //일단 left일 경우에는 prev가 있으면 해당의 right로 바꿔주면됨
        //     // top인 경우에 parent에 top이 있는 경우네느 같은 column의 윗녀석임
        //     targetElementData.uuid = parentSiblingData.prevData.uuid;
        //     targetElementData.position = "right";
        //   } else {
        //     targetElementData.uuid = parentData.uuid;
        ...
        //     }
        //   } else {
        //     if (parentSiblingData.nextData) {
        //       targetElementData.uuid = parentSiblingData.nextData.uuid;
        //       targetElementData.position = "left";
        //     } else {
        //       targetElementData.uuid = parentData.uuid;
        //     }
        //   }
        ...
        //     // const parentSiblingData = getSiblingsData(parentData);
        //     // console.log("parentSiblingData : ", parentSiblingData);
        //     // if (parentSiblingData.prevData) {
        //     //   return;
        //     // }
        //   }
