...
const CardEditor = ({ pathId }) => {
  ...
  const [nearElement, setNearElement] = useRef(null);
  const [hoverElement, setHoverElement] = useRef(null);
  const [selectElements, setSelectElements] = useRef([]);
  const [fileData, setFileData] = useRef(null);
  const [selectPoint, setSelectPoint] = useRef(null);
  const [currentPoint, setCurrentPoint] = useRef(null);
  const [contextMenuPoint, setContextMenuPoint] = useRef(null);
  ...
  const windowMouseDown = (e) => {
    ...
    console.log("contextMenuYn : ", contextMenuYn);
