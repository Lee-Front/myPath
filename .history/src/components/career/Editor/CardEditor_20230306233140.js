...
const CardEditor = ({ pathId }) => {
  ...
  function getClosestElement(elements, pos, axis) {
    ...
    console.log("rectProp : ", rectProp);
    console.log("sizeProp : ", sizeProp);

    return elements.reduce((prev, curr) => {
      const nearElRect = prev.getBoundingClientRect();
      const currRect = curr.getBoundingClientRect();
      console.log("top : ", currRect.top);
      console.log("bottom : ", currRect.bottom);
      console.log("height : ", currRect.height);

      if (currRect.top <= pos && pos <= currRect.bottom) {
        return curr;
      }

      return Math.abs(nearElRect.y - pos) > Math.abs(currRect.y - pos)
        ? curr
        : prev;
    }, elements[0]);
  }
  ...
  const findNearElementByPointer = (x, y) => {
    ...
    if (filteredContents.length > 0) {
      ...
      // nearEl = getClosestElement(equalXElements, y, "y");
      // console.log("nearEl : ", nearEl);
      // if (nearEl) {
      //   const { top, bottom } = nearEl.getBoundingClientRect();
      //   if (top <= y && y <= bottom) {
      //     hoverEl = nearEl;
      //   }
      // } else {
      ...
      //   console.log("?");
      //   nearEl = getClosestElement(equalYElements, x, "x");
      //   if (nearEl) {
      //     const { left, right } = nearEl.getBoundingClientRect();
      //     if (left <= x && x <= right) {
      //       hoverEl = nearEl;
      //     }
      //   }
      ...
      if (equalXElements.length > 0) {
        nearEl = equalXElements.reduce((prev, curr) => {
          const nearElRect = prev.getBoundingClientRect();
          const currRect = curr.getBoundingClientRect();

          if (currRect.top <= y && y <= currRect.bottom) {
            hoverEl = curr;
            return curr;
          }

          return Math.abs(nearElRect.y - y) > Math.abs(currRect.y - y)
            ? curr
            : prev;
        }, equalXElements[0]);

        console.log("nearEl: ", nearEl);
        console.log("fn : ", getClosestElement(equalXElements, y, "y"));
      } else {
        const equalYElements = filteredContents.filter((element) => {
          const { top, bottom } = element.getBoundingClientRect();
          return top <= y && y <= bottom;
        ...
        if (equalYElements.length > 0) {
          nearEl = equalYElements.reduce((prev, curr) => {
            const nearElRect = prev.getBoundingClientRect();
            const currRect = curr.getBoundingClientRect();

            if (currRect.left <= x && x <= currRect.right) {
              hoverEl = curr;
              return curr;
            }
            return Math.abs(nearElRect.x - x) > Math.abs(currRect.x - x)
              ? curr
              : prev;
          }, equalYElements[0]);
        }
