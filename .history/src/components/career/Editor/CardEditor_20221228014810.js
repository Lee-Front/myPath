      ...
      if (minDistance.position === "top" || minDistance.position === "bottom") {
        if (!hoverElement) {
          ...
          if (minDistance.position === "top") {
            ...
            if (includeElements > 0) {
              // 대상이 선택된 Element이면 다음것으로 해줘야됨
            } else {
            }
            ...
            /*
            1. includeElements가 0이고 prevData가 있다.
             => 현재 타겟팅된 Element가 select되지 않았다는것이며 prevData의 Bottom으로 보내면 된다

            2. includeElements가 0이고 prevData가 없다.
             => 현재 타겟팅된 Element가 select되지 않닸다는것이며 prevData가 없다는건
              타겟팅된 Element가 최상단에 존재한다는 것이다.

            3. includeElements가 1이고 prevData가 있다.
             => 현재 타겟팅된 Element가 select된 Element이므로 이
            4. includeElements가 1이고 prevData가 없다

            */
            if (includeElements && prevData) {
            ...
            // 다음과 이전이 없다고 바꾸는게 아니라 마지막게 select와 같으면 이전것
            targetElementData.uuid = topParentSiblingsData.nextData
              ? topParentSiblingsData.nextData.uuid
              : topParentData.uuid;
            targetElementData.position = topParentSiblingsData.nextData
              ? "top"
              : minDistance.position;
            console.log("uuid : ", topParentData.uuid);
