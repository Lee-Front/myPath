...
const CardEditor = () => {
  ...
  const modifyEditDom = (uuid, html) => {
    ...
    //newEditDom = findElementDataByUuid(newEditDom, uuid);
    console.log("be : ", newEditDom);
    //const modifyData = { ...elementData, html: html };
    newEditDom = newEditDom.map((dom) => {
      if (dom.tag === "multiple") {
        findElementFromChild(dom, uuid);
      } else {
        if (dom.uuid === uuid) {
          dom.html = html;
        }
    ...
    // console.log("afterEditDom : ", newEditDom);
    // setEditDom(() => newEditDom);
  };

  const findElementDataByUuid = (uuid) => {
    let findData;

    editDom.map((dom) => {
      const data = findElementFromChild(dom, uuid);
  ...
  const findElementFromChild = (dom, uuid) => {
    let findData;
    if (dom.tagName === "multiple") {
      dom?.multipleData?.data.map((element) => {
        const data = findElementFromChild(element, uuid);
