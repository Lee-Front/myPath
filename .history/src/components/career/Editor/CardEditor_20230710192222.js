...
const CardEditor = ({ pathId }) => {
  ...
  const findElementsByPoint = (Contents, x, y) => {
    if (Contents.length > 0) {
      ...
      if (!xAxisResults?.nearEl) {
        ...
        const hoverBlock = editorStore.findBlock(yAxisResults?.hoverEl.getAttribute("data-uuid")
        editorStore.setHoverBlock(yAxisResults?.hoverEl);
      } else {
        nearElement.current = xAxisResults?.nearEl;
        editorStore.setHoverBlock(xAxisResults?.hoverEl);
      }
    }
  };

  const decideMovementSide = (x1, y1) => {
    const targetElement = editorStore.hoverBlock || nearElement.current;
    if (!targetElement) {
      setMovementSide(null);
      return;
    }
  ...
  const toggleFileUploader = (e) => {
    const hoverData = editorStore.findBlock(
      editorStore.hoverBlock?.getAttribute("data-uuid")
    );
    ...
    if (e.button === 0 && hoverData?.tagName === "image") {
      if (!isFileUploderOpen) {
        const { bottom, left, width } =
          editorStore.hoverBlock?.getBoundingClientRect();
        let popupY = 0;
        let popupX = 0;
  ...
  const handleEditorContextMenu = (e) => {
    ...
    if (!filePopup && editorStore.hoverBlock) {
      setPopupUuid(editorStore.hoverBlock?.getAttribute("data-uuid"));
