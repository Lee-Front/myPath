...
const SubContextMenu = ({
  ...
  useEffect(() => {
    ...
    if (selectMenu && subMenuPopup) {
      ...
      const selectMenuRight = selectMenuRect.left + selectMenuRect.width;
      const selectMenuTop = selectMenuRect.top;

      const point = {
        x:
          pointX > window.innerWidth
            ? selectMenu.offsetLeft + selectMenu.offsetWidth - subMenuRect.width
            : selectMenu.offsetLeft + selectMenu.offsetWidth,
        y:
          pointY > window.innerHeight
            ? selectMenu.offsetTop - subMenuRect.height
            : selectMenu.offsetTop,
      };
      setMenuPoint(point);
    }
  }, [selectMenu]);
