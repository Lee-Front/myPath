...
const SubContextMenu = ({
  ...
  const menuList = [
    { menuId: 1, menuText: "삭제", onClick: deleteMenu },
    { menuId: 2, menuText: "변경", onClick: changeMenu },
    {
      menuId: 3,
      menuText: "정렬",
      subMenuList: [
    ...
    { menuId: 4, menuText: "링크", onClick: handleLinkClick },
  ];
    ...
    <SubContextWarpper>
      {menuList.map((item) => (
        <SubContextMenuWrapper onClick={item.onClick} key={item.menuText}>
          ...
          {selectMenu === item && item.subMenuList && (
