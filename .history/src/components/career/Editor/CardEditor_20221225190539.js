...
const CardEditor = () => {
  ...
  const findNearElementInChild = (x, y, Contents) => {
    ...
    for (let i = 0; i < Contents.length; i++) {
      ...
      const nearDistance = Math.abs(nearRect.x - x);
      const contentDistance = Math.abs(contentRect.x - x);

      // 여기부터
      if (nearDistance === contentDistance) {
        if (contentRect.top <= y && contentRect.bottom >= y) {
          nearEl = Contents[i];
        }
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    if (targetElement && selectElements.length > 0) {
      ...
      if (parentData) {
        ...
        // if (minDistance.position === "left" || minDistance.position === "top") {
        //   if (parentSiblingData.prevData) {
        //     console.log("left || top");
        //     console.log("prevData : ", parentSiblingData.prevData);
        //   }
        // } else {
