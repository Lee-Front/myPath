  ...
  return (
    ...
    >
      ...
      {isFileUploderOpen || isContextMenuOpen || draggable ? (
        ...
        >
          {editorStore.selectBlocks.length > 0 && (
            <OverlayWrapper currentPoint={currentPoint}>
              {editorStore.selectBlocks?.map((element) => {
                const target = document.querySelector(".overlayContainer");
                console.log("element: ", element);
                const reactElement = React.createElement("div", null, element);

                return target && createPortal(reactElement, testRef.current);
                // return (
                //   <EditBranchComponent
