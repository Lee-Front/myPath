...
const CardEditor = ({ pathId }) => {
  ...
  const windowMouseDown = (e) => {
    if (
      ...
      selectElements.current = makeTree(editorStore.blocks, hoverUuid);
    }
  ...
  const modifyDomSave = async (newEditDom) => {
    const editDomList = copyObjectArray(editorStore.blocks);
  ...
  const findNearElementByPointer = (Contents, x, y) => {
    if (Contents.length > 0) {
      ...
      const closestData = findClosestElementByAxis(equalXElements, y, "y");

      if (!closestData?.nearEl) {
        const equalYElements = Contents.filter((element) => {
          const { top, bottom } = element.getBoundingClientRect();
          return top <= y && y <= bottom;
        });

        const [nearEl, hoverEl] = findClosestElementByAxis(
          equalYElements,
          x,
          "x"
        );
  ...
  const deleteElement = (uuid) => {
    let editElements = copyObjectArray(editorStore.blocks);
  ...
  const changeElementStyle = (uuid, styleData) => {
    const editElements = copyObjectArray(editorStore.blocks).map((element) => {
      ...
      <CardEditorContentWrapper
        ...
        onMouseUp={(e) => {
          if (
            ...
            modifyDomSave([...editorStore.blocks, newElement]);
