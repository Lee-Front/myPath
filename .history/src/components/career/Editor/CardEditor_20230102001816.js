...
const CardEditor = ({ pathId }) => {
  ...
  const modifyDomSave = (newEditDom) => {
    console.log("editDom : ", editDom);
    console.log("newEditDom: ", newEditDom);
    const modifyList = [];
    editDom.map((element) => {
      const test = getDifferentType(element, newEditDom);
  ...
  const getDifferentType = (originalElement, newEditDom) => {
    ...
    newEditDom.map((element) => {
      if (originalElement.uuid === element.uuid) {
        console.log("originalElement: ", originalElement.key);
        console.log("keys : ", originalElement.keys);
        // 해당  elemnt
      }
    });

    return differentType ? differentType : "delete";
  };
