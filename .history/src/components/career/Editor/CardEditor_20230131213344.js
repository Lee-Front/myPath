  ...
  const moveElementData = (from, to) => {
    ...
    if (to.parentId) {
      ...
      if (
        ...
        const width = parseFloat(100 / (rowChildElements.length + 1)).toFixed(4);
    ...
    if (columnChildElements.length < 1) {
      newEditDom.map((element, index) => {
        ...
        if (element.uuid === elementData.parentId) {
          ...
          console.log("remove width : ", element.width);

          console.log("rowChildElements : ", rowChildElements);
          rowChildElements.map((rowElement) => {
            console.log("row width : ", rowElement.width);
            console.log("100 + element.width: ", 100 + element.width);
            console.log("(rowElement.width / 100) : ", rowElement.width / 100);
            console.log(
              "after : ",
              (100 + element.width) * (rowElement.width / 100)
            );
            console.log("after2 : ", element.width * (rowElement.width / 100));
          });
        }
      });
    }
