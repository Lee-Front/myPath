...
const CardEditor = ({ pathId }) => {
  ...
  const [filePosition, setFilePosition] = useState(null);
    ...
    if (movementSide?.uuid && selectElements.length > 0) {
      ...
      if (
        ...
        if (hoverData.tagName === "image") {
          const { left, bottom } = hoverElement.getBoundingClientRect();
          setFilePosition({ x: left, y: bottom });
          setPopupYn(!popupYn);
        }
      }
    }
