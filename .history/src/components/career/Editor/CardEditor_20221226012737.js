...
const CardEditor = () => {
  ...
  const findNearElementInChild = (x, y, Contents) => {
    ...
    if (equalYElements.length > 0) {
      const findNearData = findNearElement(
        x,
        y,
        equalYElements,
        nearEl,
      ...
      nearEl = findNearData?.nearEl;
      hoverEl = findNearData?.hoverEl;
    } else {
      ...
      if (equalXElements.length > 0) {
        findNearElement(x, y, equalXElements, nearEl, hoverEl);
    ...
    console.log({ nearEl, hoverEl });
    return { nearEl, hoverEl };
  };

  const findNearElement = (x, y, elements, nearEl, hoverEl) => {
  ...
  //     console.log("contentDistance : ", contentDistance);

  //     // 여기부터
  //     if (nearDistance === contentDistance) {
  //       if (contentRect.left <= x && contentRect.right >= x) {
  //         nearEl = Contents[i];
  //       }
      ...
      if (minDistance.position === "top" || minDistance.position === "bottom") {
        if (!hoverElement) {
          ...
          if (minDistance.position === "bottom") {
            targetData = includeElements ? prevData : targetData;
          } else if (minDistance.position === "top") {
            targetData = includeElements ? nextData : targetData;
          }
