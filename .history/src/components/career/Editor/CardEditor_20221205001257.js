      ...
      if (element.tagName === "multiple") {
        ...
        if (childData) {
          ...
          childData.forEach((data, index) => {
            let includesYn = from.filter((el) => el.uuid === data.uuid).length;

            if (includesYn > 0 || to.uuid === data.uuid) {
        ...
        if (includesYn > 0 || to.uuid === element.uuid) {
          newEditDom.splice(index, 1);
        }
      }
    });

    console.log("newEditDom : ", newEditDomnewEditDom);
