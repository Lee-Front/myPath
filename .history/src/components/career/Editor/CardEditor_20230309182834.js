...
const CardEditor = ({ pathId }) => {
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    const clonedEditDom = copyEditDom();
  ...
  const moveElementData = (selectDatas, movementData) => {
    ...
    const filteredDom = copyEditDom().filter((element) => {
      const isSelectedElement = selectDatas.some(
    ...
    const toIndex = findIndexByKey(filteredDom, "uuid", targetData.uuid);
    const findToData = filteredDom[toIndex];
    // 여기까지 수정했음

    // toIndex가 없을수는 없지만 이동 대상이되는 Index를 못 찾았으면 실행방지
    if (toIndex === -1) return;

    if (["top", "bottom"].includes(movementData.position)) {
      if (
        ...
        fromDatas.forEach((element) => {
          element.parentId = parentData ? parentData.uuid : findToData.uuid;
        });
      }
      filteredDom.splice(
      ...
      const rowElement = makeNewElement({
        tagName: "multiple",
        direction: "row",
      });

      const columElement1 = makeNewElement({
        tagName: "multiple",
        direction: "column",
      });

      const columElement2 = makeNewElement({
        tagName: "multiple",
        direction: "column",
        width: 50,
      });
      ...
      fromDatas.forEach((element) => {
        element.parentId = columElement1.uuid;
      });

      if (!targetData.parentId) {
        columElement1.width = 50;
        columElement1.parentId = rowElement.uuid;
        columElement2.parentId = rowElement.uuid;
        findToData.parentId = columElement2.uuid;
      }
      ...
      if (movementData.position === "left") {
        if (targetData.parentId) {
          ...
          columElement1.width = parseFloat(width);
          columElement1.parentId = targetData.parentId;
          filteredDom.splice(toIndex, 0, columElement1, ...fromDatas);
        } else {
          filteredDom.splice(
            toIndex,
            0,
        ...
        if (targetData.parentId) {
          ...
          filteredDom.splice(toIndex + 1, 0, rowElement, ...fromDatas);
        } else {
          filteredDom.splice(toIndex + 1, 0, columElement1, ...fromDatas);
          filteredDom.splice(toIndex, 0, rowElement, columElement2);
        }
      }
    }
