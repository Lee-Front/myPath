...
const CardEditor = ({ pathId }) => {
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    if (minDistance.position === "top") {
      // const data = hoverBlock ? previousSibling : topParentSiblingsData;
      // targetElementData.uuid = data.previousSibling
      //   ? data.previousSibling.uuid
      //   : topParentData.uuid;
      // targetElementData.position = data.previousSibling
      //   ? "bottom"
      ...
      if (!hoverBlock) {
        const topParentSiblingData = topParentSiblingsData.previousSibling;

        targetElementData.uuid = topParentSiblingData
          ? topParentSiblingData.uuid
          : topParentData.uuid;
        targetElementData.position = topParentSiblingData
          ? "bottom"
          : minDistance.position;
      } else {
