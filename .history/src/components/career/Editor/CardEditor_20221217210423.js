...
const CardEditor = () => {
  ...
  const findNearElementInChild = (x, y, Contents) => {
    ...
    // multiple 들은 기본적으로 대상이 아니기 때문에 제거
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    if (targetElement && selectElements.length > 0) {
      ...
      //즉 parentData가 있으며 left, right인 경우에만 따로 처리해주면됨
      ...
      if (minDistance.position === "top" || minDistance.position === "bottom") {
        // 단순 위치이동 혹은 multiple 내의 자식으로 이동
        // 우선 top인 경우에는 이전 Element가 있으면 해당 Element의 bottom으로 바꿔줘야됨
        ...
        if (parentData) {
          ...
          // 목적이 더 앞에 있는 Element의 테두리를 사용하려는건데
          //
          // 부모가 있을때는 left나 right일때
          targetElementData.uuid =
            siblingsData && siblingsData?.uuid !== parentData.uuid
              ? siblingsData.uuid
              : targetUuid;
