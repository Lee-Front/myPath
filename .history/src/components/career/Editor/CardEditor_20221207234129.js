...
const CardEditor = () => {
  ...
  const testArray = [
    ...
    { uuid: "10", html: "10", parentId: "7", tagName: "div" },
    { uuid: "11", html: "11", parentId: "7", tagName: "div" },
    {
      uuid: "1",
      html: "",
      parentId: null,
  ...
  const filterDomData = (from, to) => {
    ...
    const findFromData = editDom.filter((element) => {
      return from.filter((el) => el.uuid === element.uuid).length > 0;
    });

    console.log("findFromData : ", findFromData);
    console.log("to : ", to);

    // left, right인 경우 child로 들어가는것임
    if (movementSide.position === "left" || movementSide.position === "right") {
      console.log("합병");
    } else {
      // parent가 있는 놈한테 갈경우는 합병임
      if (to.parentId) {
      } else {
      }
      console.log("위치 이동");
