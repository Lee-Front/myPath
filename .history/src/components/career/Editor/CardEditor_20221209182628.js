...
const CardEditor = () => {
  ...
  const decideMovementSide = (x1, y1) => {
    if (nearElement && selectElements.length > 0) {
      ...
      console.log("movementSide.position : ", movementSide.position);
      const siblingData = equalParentElements.filter((element) => {
        if (
      ...
      if (minDistance.position === "top") {
        ...
        const prevData = getEditComponentData(
          prevElement?.getAttribute("uuid")
        );

        if (prevData?.uuid !== selectElements[0].uuid) {
