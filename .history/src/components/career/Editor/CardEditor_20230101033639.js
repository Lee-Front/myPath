...
const CardEditor = ({ pathId }) => {
  ...
  const getTagList = async (pathId) => {
    const data = await axios.get("/api/editor/getList", { pathId });
  ...
  const saveTagBlock = async (uuid, html) => {
    const tagData = getEditComponentData(uuid);
