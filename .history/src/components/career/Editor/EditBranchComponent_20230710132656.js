...
const EditBranchComponent = ({
  ...
  const isSelection = editorStore.selectBlocks.find(
    (block) => block.uuid === data.uuid
  );

  // if (item.tagName === "multiple") return null;
  //       if (item.parentId) {
  //         const parentBlock = editorStore.findBlock(item.parentId);
  //         if (
  ...
  //       return createPortal(<SelectionHalo key={item.uuid} />, element);
