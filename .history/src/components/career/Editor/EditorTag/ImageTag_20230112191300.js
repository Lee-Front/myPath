...
const ImageTag = ({ modifyEditDom, movementSide, data }) => {
  const file = data?.imageUrl?.getAll("image")[0];
  ...
  const fileUpload = async (file) => {
    ...
    const upload = await axios.post("/api/editor/upload", formData);
    return upload.data;
  };
            ...
            <div style={{ borderTop: "1px solid rgba(55, 53, 47, 0.3)" }}>
              ...
              >
                <input
                  ...
                  onChange={(e) => {
                    setPopupYn(!popupYn);
                    const data = fileUpload(e.target.files[0]);
                    setImageUrl(data.url);
                    modifyEditDom(data.uuid, { imageUrl: data.url });
                  }}
                />
                파일 업로드
              </div>
