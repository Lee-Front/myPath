...
const ImageTag = ({ modifyEditDom, movementSide, data }) => {
  ...
  const fileUpload = async (file) => {
    ...
    const upload = await axios.post("/api/common/upload", formData,{headers:{"Content-Type": "multipart/form-data; charset=UTF-8"}});

    headers: { "Content-Type": `application/json`}
    setImageUrl(upload.data.url);
    modifyEditDom(data.uuid, { imageUrl: upload.data.url });
  };
