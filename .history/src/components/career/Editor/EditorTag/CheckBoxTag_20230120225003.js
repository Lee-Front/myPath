...
`;
const CheckBoxTag = ({ style, data, hoverElement }) => {
  const hoverUuid = hoverElement?.getAttribute("uuid");
  const [checkYn, setCheckYn] = useState(false);
  const [hoverYn, setHoverYn] = useState(hoverUuid == data.uuid);
  console.log("hoverUuid == data.uuid : ", hoverUuid == data.uuid);
  console.log("hoverYn : ", hoverYn);
  const editRef = useRef(null);

  return (
    <div uuid={data.uuid}>
      <CheckBoxTagWrapper hoverYn={hoverYn}>
