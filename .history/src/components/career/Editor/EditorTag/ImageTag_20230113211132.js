...
const ImageTag = ({ modifyEditDom, movementSide, data }) => {
  ...
  const fileUpload = async (file) => {
    ...
    const upload = await axios.post("/api/common/upload", formData, data.uuid);
    setImageUrl(upload.data.fileId);
    modifyEditDom(data.uuid, { imageUrl: upload.data.fileId });
  };
  ...
  return (
    <div uuid={data.uuid} style={{ position: "relative" }}>
      ...
      >
        {imageUrl ? (
          <img
            src={`http://localhost:8080/api/common/images/${imageUrl}`}
            alt="이미지"
            style={{ width: "100%" }}
          />
        ) : (
