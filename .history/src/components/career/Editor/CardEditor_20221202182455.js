...
const CardEditor = () => {
  ...
  const modifyEditDom = useCallback(
    (uuid, html) => {
      ...
      const elementData = findElementDataByUuid(uuid);
  ...
  const findElementDataByUuid = (uuid) => {
    ...
    const findElement = editDom.forEach((dom) => {
      const findData = findElementFromChild(dom, uuid);
      if (findData?.uuid === uuid) {
        console.log("findElement : ", findData);
        return findData;
      }
    });
  ...
  const findElementFromChild = (dom, uuid) => {
    if (dom.tagName === "multiple") {
      dom?.multipleData?.data.map((element) => {
        findElementFromChild(element, uuid);
      });
    } else {
      if (dom.uuid === uuid) {
        return dom;
