...
const CardEditor = ({ pathId }) => {
  ...
  const findAllChildUuids = (elements, uuid) => {
    ...
    while (stack.length > 0) {
      ...
      for (let i = 0; i < elements.length; i++) {
        ...
        if (item.parentId === currUuid) {
          subData.push(item.uuid);
          if (!stack.includes(item.uuid)) {
            stack.push(item.uuid);
          }
        }
      }
    }
    ...
    //     subData.push(item.uuid);
    //     const subChildData = findAllChildUuids(item.uuid);
  ...
  const modifyEditDom = (uuid, data, type) => {
    ...
    if (type === "delete") {
      const childList = findAllChildUuids(newEditDom, uuid);
