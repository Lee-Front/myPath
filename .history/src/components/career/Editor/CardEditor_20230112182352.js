...
const CardEditor = ({ pathId }) => {
  ...
  const modifyDomSave = async (newEditDom) => {
    ...
    editDom.map((element) => {
      ...
      if (differentData.type) {
        ...
        if (differentData.data.tagName === "image") {
          console.log("differentData.imageUrl: ", differentData.imageUrl);
          imageList.push(differentData.imageUrl);
        }
      }
    });
    ...
    console.log("file : ", imageList[0]);
    await axios.post("/api/editor/upload", imageList[0]);
  };
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    if (targetElement && selectElements.length > 0) {
      ...
      for (let i = 0; i < selectElements.length; i++) {
        ...
        // else if (
        //   minDistance.position === "top" &&
        //   topSiblingsData.prevData?.uuid === selectElements[i].uuid
        // ) {
        //   setMovementSide(null);
        //   return;
        // }
  ...
  const modifyEditDom = (uuid, data) => {
    ...
    newEditDom = newEditDom.map((dom) => {
      ...
      } else if (dom.tagName === "image") {
        dom.imageUrl = data.imageUrl;
      } else {
        if (dom.uuid === uuid) {
          dom.html = data.html;
        }
  ...
  return (
    <EditorWrapper>
      ...
      <CardEditorContentWrapper
        ref={editorRef}
        onMouseUp={(e) => {
          if (e.target === e.currentTarget && !draggable && !hoverElement) {
            const newUuid = uuidv4();
