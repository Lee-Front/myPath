...
const CardEditor = ({ pathId }) => {
  ...
  const moveElementData = (from, to) => {
    ...
    const fromDatas = newEditDom.filter((element, index) => {
      if (from.some((obj) => element.uuid === obj.uuid)) {
        newEditDom.slice(index, 1);
        return element;
      }
    });
    ...
    console.log("newEditDom: ", ...newEditDom);
    newEditDom.map((element, index) => {
      if (element.uuid === from[0].uuid) {
        fromData = newEditDom.splice(index, 1)[0];
      }
    });
