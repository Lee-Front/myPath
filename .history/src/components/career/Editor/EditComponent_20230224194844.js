import React, { useState } from "react";
import styled from "@emotion/styled";
import { useRef } from "react";
import { memo } from "react";

const EditableTag = styled.div`
  position: relative;
  outline: none;
  line-height: 1.5;
  padding: 0.2rem 0.4rem;
  word-break: break-all;
  //white-space: pre-wrap;
`;

const EditComponent = ({
  modifyEditDom,
  data,
  style,
  overlayWidth,
  hoverUuid,
}) => {
  const [html, setHtml] = useState(data?.html);
  const [editPlaceHolder, setEditPlaceHolder] = useState(null);
  const editRef = useRef(null);

  const setCaretPosition = (target, startOffset) => {
    //const nodes = target.childNodes;
    // const { startNodeIndex, endNodeIndex, startOffset, endOffset } =
    //   newHtmlData;
    const newRange = document.createRange();
    // let startNode = nodes[startNodeIndex];
    // let endNode = nodes[endNodeIndex];

    // if (startNode.nodeName === "SPAN") {
    //   startNode = startNode.firstChild;
    // }

    // if (endNode.nodeName === "SPAN") {
    //   endNode = endNode.firstChild;
    // }

    newRange.setStart(target, startOffset);
    console.log("a");
    //newRange.setEnd(target, startOffset);
    document.getSelection().removeAllRanges();
    document.getSelection().addRange(newRange);
  };

  return (
    <div
      uuid={data.uuid}
      style={{
        position: "relative",
        width: overlayWidth + "%",
      }}
    >
      <EditableTag
        hoverYn={hoverUuid === data.uuid}
        ref={editRef}
        className="editable-tag"
        contentEditable={true}
        suppressContentEditableWarning={true}
        onDragStart={(e) => {
          e.preventDefault();
        }}
        placeholder={editPlaceHolder}
        onFocus={() => {
          setEditPlaceHolder("내용을 입력하세요");
        }}
        onBlur={() => {
          setEditPlaceHolder(null);
        }}
        onInput={(e) => {
          e.preventDefault();
          const childNodes = Array.from(e.target.childNodes);
          let newHtml = "";

          // 노드가 2개이상임
          if (childNodes.length > 1) {
            for (let i = 0; i < childNodes.length; i++) {
              if (childNodes[i]?.nodeName === "SPAN") {
                // 현재와 뒤에것 전부 text이니까 합쳐주면됨
                newHtml += childNodes[i].outerHTML;
              } else {
                newHtml += childNodes[i].textContent;
              }
            }
            const range = window.getSelection().getRangeAt(0);

            const startIndex = childNodes.indexOf(range.startContainer);
            const startOffset = range.startOffset;
            const target = document.querySelector(`[uuid="${data.uuid}"]`);

            //setHtml(newHtml);

            const editableTag = target.querySelector(".editable-tag");
            console.log("editableTag: ", editableTag.innerHTML);
            editableTag.innderHTML = newHtml;
            const newChildeList = editableTag.childNodes;
            setCaretPosition(
              newChildeList[startIndex],
              startOffset,
              startOffset
            );

            modifyEditDom(data.uuid, {
              html: newHtml,
            });
          } else {
            modifyEditDom(data.uuid, {
              html: e.target.innerHTML,
            });
          }
        }}
        dangerouslySetInnerHTML={{ __html: html }}
        suppressHydrationWarning={true}
      />
      {hoverUuid === data.uuid ? (
        <div
          style={{
            position: "absolute",
            width: "100%",
            height: "100%",
            top: 0,
            pointerEvents: "none",
            background: "rgba(55, 53, 47, 0.1)",
          }}
        ></div>
      ) : null}

      {style ? <div style={style}></div> : null}
    </div>
  );
};

export default memo(EditComponent);
