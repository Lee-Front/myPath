...
const CardEditor = ({ pathId }) => {
  ...
  const findTargetIndex = (elementList, targetData, findKey) => {
    console.log("findKey: ", findKey);
    elementList.findIndex((element) => {
      console.log("element: ", element[findKey]);
      console.log("targetData: ", targetData[findKey]);
      return element[findKey] === targetData[findKey];
    });
  };
  // =================여기까지 수정완료======================= //
  ...
  const moveElementData = (selectDatas, movementData) => {
    ...
    // });
    const toIndex = findTargetIndex(filteredDom, targetData, "uuid");

    console.log("toIndex: ", toIndex);
    const findToData = filteredDom.filter((element) => {
      return targetData.uuid === element.uuid;
    })[0];
    console.log("findToData: ", findToData);
    console.log("test : ", filteredDom[toIndex]);
    // const toIndex = findTargetIndex(filteredDom, targetData, "uuid");
