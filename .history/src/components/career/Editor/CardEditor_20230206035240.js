...
const CardEditor = ({ pathId }) => {
  ...
  const windowMouseUp = (e) => {
    ...
    if (movementSide?.uuid && selectElements.length > 0) {
      ...
      const { clientX, clientY } = e;
      const currentSelectPoint = { ...selectPoint };
      console.log("currentSelectPoint: ", currentSelectPoint);
      const distance = Math.sqrt(
        Math.pow(Math.abs(clientX - currentSelectPoint.x), 2) +
          Math.pow(Math.abs(clientY - currentSelectPoint.y), 2)
      );
