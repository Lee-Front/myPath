...
const CardEditor = ({ pathId }) => {
  ...
  function getClosestElement(elements, pos, axis) {
    ...
    return elements.reduce((prev, curr) => {
      ...
      if (
        prevRect[rectProp] <= pos &&
        pos <= prevRect[rectProp] + prevRect[sizeProp]
      ) {
        return curr;
      }

      const prevDist = Math.abs(prevRect[rectProp] - pos);
      const currDist = Math.abs(currRect[rectProp] - pos);
      console.log("pos : ", pos);
      console.log("prevDist : ", prevRect[rectProp]);
      console.log("currDist : ", currRect[rectProp]);

      console.log("test : ", currDist < prevDist ? curr : prev);
