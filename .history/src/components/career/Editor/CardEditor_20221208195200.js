...
const CardEditor = () => {
  ...
  const changeArrayOrder = (list, target, moveValue) => {
    if (list.length < 0) return;

    const targetIdx = list.findIndex((element) => {
      return element.uuid === target.uuid;
    });
    ...
    const tempList = list.map((element) => Object.assign({}, element));
