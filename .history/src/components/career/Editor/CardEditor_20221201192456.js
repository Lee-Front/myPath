...
const CardEditor = () => {
  ...
  const decideMovementSide = (x1, y1) => {
    if (hoverElement && selectElements.length > 0) {
      const hoverUuid = hoverElement.getAttribute("uuid");

      for (let i = 0; i < selectElements.length; i++) {
        if (hoverUuid === selectElements[i].uuid) {
          setMovementSide(null);
          return;
        }
      }

      const { bottom, left, right } = hoverElement.getBoundingClientRect();
      ...
      if (minDistance.position === "top") {
        const prevElement = hoverElement.previousSibling;

        targetElementData.uuid = prevElement
          ? hoverElement.previousSibling.getAttribute("uuid")
          : hoverUuid;
        ...
        targetElementData.uuid = hoverUuid;
  ...
  return (
    <EditorWrapper>
      ...
      >
        {editDom.map((element) => (
          <EditBranchComponent
            hoverUuid={!draggable && hoverElement?.getAttribute("uuid")}
            key={element.uuid}
            uuid={element.uuid}
