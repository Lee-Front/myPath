  ...
  return (
    ...
    >
      ...
      {editorStore.selectBlocks.map((item) => {
        if (item.parentId) {
          const parent = editorStore.findBlock(item.parentId);
          if (parent.tagName === "multiple") return null;
          if (parent.tagName === "checkbox") return null;
          if (parent.tagName === "bullet") return null;
        }
