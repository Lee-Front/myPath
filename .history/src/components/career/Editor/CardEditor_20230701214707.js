...
const CardEditor = ({ pathId }) => {
  ...
  const findElementsByPoint = (Contents, x, y) => {
    ...
    if (Contents.length > 0) {
      // 마우스가 x범위에 해당하는 blocks만 추려냄
      const equalXElements = Contents.filter((element) => {
        const { left, right } = element.getBoundingClientRect();
        return left <= x && x <= right;
      });

      const resultByY = findElementByAxis(equalXElements, y, "y");
      ...
      if (!resultByY?.nearEl) {
        ...
        const resultByX = findElementByAxis(equalYElements, x, "x");
        nearElement.current = resultByX?.nearEl;
        hoverElement.current = resultByX?.hoverEl;
        nearBlock = resultByX?.nearEl;
        hoverBlock = resultByX?.hoverEl;
      } else {
        nearElement.current = resultByY?.nearEl;
        hoverElement.current = resultByY?.hoverEl;
        nearBlock = resultByY?.nearEl;
        hoverBlock = resultByY?.hoverEl;
      }
    }
