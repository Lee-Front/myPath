...
const CardEditor = () => {
  ...
  const windowMouseUp = () => {
    if (movementSide && selectElements.length > 0) {
      ...
      newEditDom = filterDomData(newEditDom, selectElements, hoverData);
  ...
  const filterDomData = (dom, from, to) => {
    // 1. 옮겨질 domData
    // 2. 옮겨질 위치의 domData
    ...
    dom = dom.filter((element) => {
      ...
      if (element.tagName === "multiple") {
        ...
        if (childData.length === 1) {
          ...
          element.tagName = "div";
          element.html = childData[0].html;
          console.log("element : ", element);
          element.multipleData = undefined;
        } else {
          element.multipleData.data = childData;
        }

        const includesYn = from.filter((el) => el.uuid === element.uuid).length;

        if (includesYn <= 0 && element.multipleData.data.length > 0) {
          return element;
        }
      } else {
        const includesYn = from.filter((el) => el.uuid === element.uuid).length;
