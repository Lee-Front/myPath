  ...
  const decideMovementSide = (x1, y1) => {
    ...
    if (targetElement && selectElements.length > 0) {
      ...
      const parentData = targetData?.parentId
        ? getEditComponentData(targetData.parentData)
        : null;
      console.log("targetData?.parentId : ", targetData?.parentId);
      ...
      console.log(targetData.parentId);
      console.log("parentData : ", parentData);
      // 부모 Element가 존재함 즉 row, colum 데이터중 하나일것임
      if (parentData) {
