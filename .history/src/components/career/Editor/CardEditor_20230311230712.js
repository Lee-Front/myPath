...
const CardEditor = ({ pathId }) => {
  ...
  const changeElementStyle = (elements, uuid, styleData) => {
    const editElements = copyObjectArray(elements);
    const targetElement = editElements.map((element) => {
      if (element.uuid === uuid) {
        element.styleData = styleData;
      }
    });

    if (targetElement.length > 0) {
      targetElement[0].styleData = styleData.styleData;
    }
