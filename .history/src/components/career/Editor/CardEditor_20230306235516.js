...
const CardEditor = ({ pathId }) => {
  ...
  const getClosestElement = (elements, pos, axis, nearEl, hoverEl) => {
    if (!elements || elements.length === 0) {
      return null;
    }
    //let hoverEl = undefined;

    const rectProp = axis === "x" ? "left" : "top";
    const sizeProp = axis === "x" ? "width" : "height";

    nearEl = elements.reduce((prev, curr) => {
  ...
  const findNearElementByPointer = (x, y) => {
    ...
    if (filteredContents.length > 0) {
      ...
      getClosestElement(equalXElements, y, "y", nearEl, hoverEl);
      ...
      if (!nearEl) {
        ...
        getClosestElement(equalYElements, x, "x", nearEl, hoverEl);
      }
