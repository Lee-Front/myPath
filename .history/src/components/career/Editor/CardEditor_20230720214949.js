...
const CardEditor = ({ pathId, readonly }) => {
    ...
    if (!readonly) {
      const keyDown = (e) => {
        if (e.key === "Escape") {
          ...
          if (baseNode) {
            console.log("baseNode.nodeType : ",baseNode.nodeType)
            
            const startNode =baseNode.nodeType === 3 ?  range.startContainer.parentNode;
            const blockElement = baseNode.closest("[data-uuid]");
            const blocks = JSON.parse(JSON.stringify(editorStore.blocks));
            const block = blocks.find(
              (block) => block.uuid === blockElement?.getAttribute("data-uuid")
            );
            ...
            nodes[splited.splitedDragInfo.startNodeIndex - 1].textContent =
              splitedText;
  ...
  mouseEventRef.current.mouseMove = (e) => {
    ...
    if (
      isGrabbing &&
      (clientY > editorRect.bottom || clientY < editorRect.top)
    ) {
      const delta =
        clientY -
        (clientY < editorRect.top ? editorRect.top : editorRect.bottom);
