...
const EditableComponent = ({ data }) => {
  ...
  const handleKeyDown = (e) => {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      console.log("editRef.current.childNodes: ", editRef.current.childNodes);
      const nodes = textStyler.getNodesData(
        Array.from(editRef.current.childNodes)
      );
      console.log("nodes : ", nodes);
      const splited = textStyler.splitNodes(nodes, window.getSelection());
      ...
      // const { baseNode, baseOffset } = selection;

      // if (baseNode.nodeType === 3) {
      //   const text = baseNode.textContent;
      //   const textBefore = text.slice(0, baseOffset);
      //   const textAfter = text.slice(baseOffset);
      //   baseNode.textContent = textBefore;
      //   block.html = textAfter;
      // } else {
      // }

      // blocks.splice(index + 1, 0, block);
      // editorStore.saveBlocks(blocks);
      // textStyler.setCaretPosition();
    }
  };
