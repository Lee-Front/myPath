...
const CardEditor = ({ pathId }) => {
  ...
  const findElementByAxis = (elements, pos, axis) => {
    ...
    const nearEl = elements.reduce((prev, curr) => {
      ...
      if (currRect[rectStart] <= pos && pos <= currRect[rectEnd]) {
        hoverEl = curr;
        return curr;
      }
      console.log("prevRect[rectStart]: ", prevRect[rectStart]);
      console.log("currRect[rectStart]: ", currRect[rectStart]);
      console.log("pos : ", pos);

      return Math.abs(prevRect[rectStart] - pos) >
        Math.abs(currRect[rectStart] - pos)
        ? curr
        : prev;
    }, elements[0]);
  ...
  const findElementsByPoint = (Contents, x, y) => {
    ...
    if (Contents.length > 0) {
      ...
      // });
      // const test = findElementByAxis(Contents, x, "x");
      //console.log("test: ", test);

      const closestDataByY = findElementByAxis(Contents, y, "y");
      console.log("closestDataByY: ", closestDataByY);
      ...
      if (!closestDataByY?.nearEl) {
        ...
        const closestDataByX = findElementByAxis(Contents, x, "x");
        nearElement.current = closestDataByX?.nearEl;
        hoverElement.current = closestDataByX?.hoverEl;
        nearBlock = closestDataByX?.nearEl;
        //handleBlock = closestDataByX?.nearEl;
        hoverBlock = closestDataByX?.hoverEl;
      } else {
        nearElement.current = closestDataByY?.nearEl;
        hoverElement.current = closestDataByY?.hoverEl;
        handleBlock = closestDataByY?.nearEl;
        nearBlock = closestDataByY?.nearEl;
        hoverBlock = closestDataByY?.hoverEl;
      }
    }
  ...
  const handleEditorClick = (e) => {
    console.log("hoverElement.current; ", hoverElement.current);
    if (
      e.button === 0 &&
