  ...
  const removeColumnAndRowIfEmpty = (elements) => {
    ...
    if (columns.length) {
      columns.forEach((columnElement) => {
        const columnChilds = filterByKey(
          copyElements,
          "parentId",
          columnElement.uuid
        ).length;
        ...
        if (!columnChilds) {
          copyElements = filterByKey(copyElements, "!uuid", columnElement.uuid);

          // 여기가 column이 삭제된것
          copyElements.forEach((obj) => {
            if (obj.parentId === columnElement.parentId) {
              const newWidth = (obj.width / (100 - columnElement.width)) * 100;
              obj.width = newWidth;
            }
          });
        }
    ...
    if (rows.length) {
      rows.forEach((element) => {
        const rowChilds = filterByKey(copyElements, "parentId", element.uuid);

        if (rowChilds.length === 1) {
          const rowUuid = element.uuid;
          const columnUuid = rowChilds[0].uuid;
