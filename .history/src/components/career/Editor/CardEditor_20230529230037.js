...
const CardEditor = ({ pathId }) => {
  ...
  //const [editDom, setEditDom] = useState(editorStoreRef.curent.blocks);
  ...
  // modifyDomSave함수를 개선해줘
  const modifyDomSave = async (newEditDom) => {
    const editDomList = copyObjectArray(editorStoreRef.curent.blocks);
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    const clonedEditDom = copyObjectArray(editorStoreRef.curent.blocks);
  ...
  const getEditComponentData = (uuid) => {
    const elements = copyObjectArray(editorStoreRef.curent.blocks);
  ...
  const deleteElement = (uuid) => {
    let editElements = copyObjectArray(editorStoreRef.curent.blocks);
  ...
  const changeElementStyle = (uuid, styleData) => {
    const editElements = copyObjectArray(editorStoreRef.curent.blocks).map(
      (element) => {
      ...
      <CardEditorContentWrapper
        ...
        onMouseUp={(e) => {
          if (
            ...
            modifyDomSave([...editorStoreRef.curent.blocks, newElement]);
            setNewUuid(newElement.uuid);
          }
        }}
      >
        {makeTree(editorStoreRef.curent.blocks).map((element) => {
          return (
            <EditBranchComponent
              key={element.uuid}
              data={element}
