...
const CardEditor = ({ pathId }) => {
  ...
  const deleteElement = (elements, uuid) => {
    let editElements = copyObjectArray(elements);
  ...
  const changeElementStyle = (elements, uuid, styleData) => {
    const editElements = copyObjectArray(elements).map((element) => {
      if (element.uuid === uuid) {
        return { ...element, styleData: styleData };
      }
      return element;
    });
  ...
  const updateElement = (elements, uuid, data) => {
    let editElements = copyObjectArray(elements);
    const keys = Object.keys(data);

    editElements = editElements.map((element) => {
      if (element.uuid === uuid) {
        if (keys.length > 0) {
          keys.map((key) => {
