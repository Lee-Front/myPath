...
const CardEditor = ({ pathId }) => {
  ...
  const windowMouseUp = (e) => {
    ...
    if (hoverElement.current && e.ctrlKey && !draggable) {
      console.log("hoverElement.current : ", hoverElement.current);
      const hoverBlock = editorStore.findBlock(
        hoverElement.current.getAttribute("data-uuid")
      );
      const isSelected = editorStore.selectBlocks.includes(
        editorStore.findBlock(hoverElement.current.getAttribute("data-uuid"))
      );
    }
