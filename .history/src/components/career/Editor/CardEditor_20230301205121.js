...
const CardEditor = ({ pathId }) => {
  ...
  const findAllSubData = (uuid) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (item.parentId === parentId) {
        subData.push(item);
        const subSubData = findAllSubData(data, item.uuid);
  ...
  const modifyEditDom = (uuid, data, type) => {
    ...
    if (type === "delete") {
      const treeData = makeTree(editDom, uuid);
      const nodeList = [];

      data.map((data) => {
        if (data.multipleData.length > 0) {
        }
      });
      // dele의 경우에는 자식으로 존재하는것들 전부 삭제 처리해야됨
