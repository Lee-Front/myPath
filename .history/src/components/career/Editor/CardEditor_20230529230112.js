...
const CardEditor = ({ pathId }) => {
  ...
  //const [editDom, setEditDom] = useState(editorStoreRef.current.blocks.blocks);
  ...
  const modifyDomSave = async (newEditDom) => {
    const editDomList = copyObjectArray(editorStoreRef.current.blocks.blocks);
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    const clonedEditDom = copyObjectArray(editorStoreRef.current.blocks.blocks);
  ...
  const getEditComponentData = (uuid) => {
    const elements = copyObjectArray(editorStoreRef.current.blocks.blocks);
  ...
  const moveElementData = (selectDatas, movementData) => {
    ...
    const filteredElements = copyObjectArray(
      editorStoreRef.current.blocks.blocks
    ).filter((element) => {
  ...
  const deleteElement = (uuid) => {
    let editElements = copyObjectArray(editorStoreRef.current.blocks.blocks);
  ...
  const changeElementStyle = (uuid, styleData) => {
    const editElements = copyObjectArray(
      editorStoreRef.current.blocks.blocks
    ).map((element) => {
  ...
  return (
    ...
    >
      ...
      >
        {makeTree(editorStoreRef.current.blocks.blocks).map((element) => {
          return (
            <EditBranchComponent
              key={element.uuid}
              data={element}
