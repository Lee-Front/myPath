...
const CardEditor = ({ pathId }) => {
  ...
  const filterByKey = (elementList, key, value) => {
    ...
    return elementList.filter((element) =>
      isLogicalNot ? element[key] !== value : element[key] === value
    );
  };
    ...
    if (rowElements.length) {
      rowElements.forEach((element) => {
        ...
        if (rowChildElements.length === 1) {
          ...
          // row없애야하고
          // row의 유일한 column도 제거하고 column의 자식 Element들 parentId null로
          copyElements = filterByKey(copyElements, "!parentId", rowUuid);
          //copyElements.filter((obj) => obj.parentId !== rowUuid);
