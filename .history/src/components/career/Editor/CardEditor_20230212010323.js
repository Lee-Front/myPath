...
const CardEditor = ({ pathId }) => {
  ...
  const modifyEditDomStyle = (uuid, data) => {
    let newEditDom = editDom.map((element) => Object.assign({}, element));

    const targetElement = newEditDom.filter((element) => element.uuid === uuid);

    if (targetElement.length > 0) {
      console.log("targetElement: ", targetElement[0]);
      targetElement[0].styleData = data;
    }
          ...
          <OverlayContentContents>
            ...
            {contextMenuYn ? (
              <ContextMenuPopup
                ...
                modifyEditDomStyle={modifyEditDomStyle}
