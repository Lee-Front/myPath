...
const CardEditor = ({ pathId }) => {
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    // top인 경우에 이전 블록이 있으면 이전의 bottom으로 해줘야함
    // bottom의 경우에는 그대로 사용함
    if (hoverBlock) {
      siblingBlock =
        minDistance.position === "top" ? previousSibling : targetBlock;
    } else {
      const siblingSide =
        minDistance.position === "top" ? "previousSibling" : "nextSibling";
      siblingBlock = topParentSiblingsData[siblingSide];
    }
