...
const CardEditor = () => {
  ...
  const findNearElementInChild = (x, y, Contents) => {
    ...
    for (let i = 0; i < Contents.length; i++) {
      ...
      if (nearDistance === contentDistance) {
        const nearXDistance = Math.abs(nearRect.x - x);
        const contentXDistance = Math.abs(contentRect.x - x);

        if (nearXDistance >= contentXDistance) {
          nearEl = Contents[i];
        }
      ...
      if (minDistance.position === "top" || minDistance.position === "bottom") {
        if (!hoverElement) {
          ...
          if (minDistance.position === "bottom") {
            equalData = prevData ? prevData : equalData;
          } else if (minDistance.position === "top") {
            equalData = nextData ? nextData : equalData;
          }

          const topParentdata = getTopParentData(equalData);
  ...
  const moveElementData = (from, to) => {
    ...
    if (from[0].parentId) {
      removeNullMultipleTag(newEditDom, from[0]);
    }
    ...
    //       if (element.uuid === from[0].parentId) {
    //         newEditDom.splice(index, 1);
    //         if (element.parentId) {
    //           const parentData = getEditComponentData(element.parentId);
    //           console.log("parentData : ", parentData);
    //           newEditDom.map((rowElement, index) => {
  ...
  const removeNullMultipleTag = (newEditDom, elementData) => {
    // 동일 column에 Data가 있는지 체크
    const columnChildElements = newEditDom.filter(
      (element) => element.parentId === elementData.parentId
    );
    ...
    // 무조건 최상위는 row multiple임
    const rowData = getTopParentData(elementData.parentId);

    console.log("rowData: ", rowData);
    const rowChildElements = newEditDom.filter(
      (element) => element.parentId === rowData.uuid
    );

    console.log("rowData : ", rowData);

    console.log("newEditDom: ", newEditDom);
    console.log("childElements : ", rowChildElements);
