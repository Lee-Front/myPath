...
const CardEditor = ({ pathId }) => {
  ...
  return (
    ...
    >
      ...
      */}
      {popupYn ? "popupYn true" : "popupYn false"}
      {contextMenuYn ? "contextMenuYn true" : "contextMenuYn false"}
      {draggable ? "draggable true" : "draggable false"}
      {selectElements.length ? "true" : "false"}
      {selectElements.length > 0 && draggable}
      {popupYn || contextMenuPoint || (selectElements.length > 0 && draggable)
        ? "true"
        : "false"}
      <OverlayWrapper
        zindex={
