...
const CardEditor = () => {
  ...
  const windowMouseDown = (e) => {
    if (hoverElement && e.ctrlKey) {
      ...
      console.log("data : ", getEditComponentData(hoverUuid));
  ...
  useEffect(() => {
    editorRef.current.addEventListener("mousedown", windowMouseDown);
    window.addEventListener("mouseup", windowMouseUp);
    window.addEventListener("mousemove", windowMouseMove);
    return () => {
      editorRef.current.removeEventListener("mousedown", windowMouseDown);
          ...
          <div ref={overlayRef} style={{ position: "absolute", width: "100%" }}>
            {draggable &&
              selectElements?.map((element) => {
                console.log("element : ", { ...element, uuid: null });
                return (
                  <EditBranchComponent
                    key={element.uuid}
