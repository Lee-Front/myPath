...
const CardEditor = () => {
  ...
  const decideMovementSide = (x1, y1) => {
    if (nearElement && selectElements.length > 0) {
      ...
      // 같은 depth의 element면 상관없음
      // multiple의 child로 들어간 관계면 복잡해짐
      // 같은 부모를 가진 애들끼리만 비교하면됨
      let targetElementData = {};

      const equalParentElements = editDom.filter((element) => {
        return element.parentId === selectElements[0].parentId;
      });

      const selectIndex = equalParentElements.findIndex(
        (element) => element.uuid === selectElements[0].uuid
      );
      ...
      if (minDistance.position === "top" || minDistance.position === "left") {
        siblingData = equalParentElements[selectIndex - 1];
      } else {
        siblingData = equalParentElements[selectIndex + 1];
      }
      ...
      if (minDistance.position === "top") {
        if (siblingData?.uuid !== selectElements[0].uuid) {
          targetElementData.uuid =
            siblingData.uuid !== nearUuid ? siblingData.uuid : nearUuid;

          targetElementData.position =
            siblingData.uuid !== nearUuid ? "bottom" : minDistance.position;
        }
      } else {
