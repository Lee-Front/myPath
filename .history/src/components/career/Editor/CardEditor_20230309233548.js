...
const CardEditor = ({ pathId }) => {
  ...
  const moveElementData = (selectDatas, movementData) => {
    ...
    if (targetData.parentId) {
      ...
      if (
        ...
        if (movementData?.movementSideType === "text") {
          ...
          filteredDom.splice("top" ? toIndex : toIndex + 1, 0, ...fromDatas);
        } else {
    ...
    if (selectDatas[0].parentId) {
      ...
      if (
        ...
        removeColumnAndRowIfEmpty(filteredDom, selectDatas[0].uuid);
      }
    }
  ...
  const removeColumnAndRowIfEmpty = (newEditDom, uuid) => {
    ...
    if (columnChildElements.length < 1) {
      ...
      newEditDom.forEach((element, index) => {
        ...
        } else if (element.uuid === rowUuid) {
          rowChildElements = filterByKey(newEditDom, "parentId", rowUuid);
          if (rowChildElements.length < 2) {
            rowIndex = index;
            element.parentId = null;
          }
        } else if (element.parentId === rowUuid) {
          if (element.uuid !== columnData.uuid) {
            const newWidth = (element.width / (100 - columnData.width)) * 100;
            element.width = newWidth;
          }
        }
      });
      ...
      if (columnIndex !== -1) {
        editDom.splice(columnIndex, 1);
      }
      
      if (rowIndex !== -1) {
        editDom.splice(rowIndex, 1);
      }
      
      if (rowChildElements?.length === 1) {
        const columnUuid = rowChildElements[0].uuid;
      
        editDom.forEach((element, index) => {
          if (element.uuid === columnUuid) {
            // column 삭제
            editDom.splice(index, 1);
            element.parentId = null;
          } else if (element.parentId === columnUuid) {
            // column 내의 데이터 삭제
            element.parentId = null;
          }
        });
  ...
  const modifyEditDom = (uuid, data, type) => {
    ...
    if (type === "delete") {
      ...
      removeColumnAndRowIfEmpty(newEditDom, uuid);
