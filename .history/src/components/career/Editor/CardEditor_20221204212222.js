...
const test = [
  { uuid: "1", html: "1", tagName: "div" },
  {
    ...
    multipleData: {
      direction: "row",
      data: [
        { uuid: "3", html: "3", tagName: "div" },
        { uuid: "4", html: "4", tagName: "div" },
      ],
    },
  },
];
...
const CardEditor = () => {
  ...
  const findNearElementByPointer = (x, y) => {
    ...
    if (Contents.length > 0) {
      ...
      setHoverElement(getEditComponentData(hoverEl));
    }
  };
  ...
  const getEditComponentData = (element) => {
    if (element) {
      ...
      const findData = editDom.filter((el) => {
        if (el.tagName === "multiple") {
          return getEditComponentDataInMultiple(el.multipleData.data, uuid);
        } else {
  ...
  const getEditComponentDataInMultiple = (element, uuid) => {
    const filterData = element.filter((el) => {
      if (el.tagName === "multiple") {
