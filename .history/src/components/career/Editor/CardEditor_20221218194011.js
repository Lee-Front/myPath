...
const CardEditor = () => {
  ...
  const decideMovementSide = (x1, y1) => {
    ...
    if (targetElement && selectElements.length > 0) {
      ...
      // let siblingsData;
      const { prevData, nextData } = getSiblingsData(equalData.uuid);
      ...
      if (parentData) {
        if (parentData.direction === "column") {
          ...
          console.log(getSiblingsData(parentData.parentId));
        }
        //targetElementData.uuid = parentData.uuid;
      }
  ...
  const getSiblingsData = (uuid) => {
    const equalParentElements = editDom.filter(
      (element) => element.parentId === uuid
    );

    console.log("equalParentElements : ", equalParentElements);
