...
const CardEditor = ({ pathId }) => {
  ...
  const realtimeState = {
    editDomRef: useRef([]),
    movementSideRef: useRef(""),
    nearElement: useRef(null),
    hoverElement: useRef(null),
    selectElements: useRef([]),
    fileData: useRef(null),
    selectPoint: useRef(null),
    contextMenuPoint: useRef(null),
  };
  ...
  useEffect(() => {
    realtimeState.editDomRef.current = editDom;
  }, [editDom]);

  useEffect(() => {
    realtimeState.movementSideRef.current = movementSide;
  }, [movementSide]);
  ...
  const copyEditDom = () => {
    ...
    return realtimeState.editDomRef.current.map((element) =>
  ...
  const windowMouseDown = (e) => {
    if (
      !popupYn &&
      !contextMenuYn &&
      realtimeState.hoverElement.current &&
      e.ctrlKey
    ) {
      window.getSelection().removeAllRanges();

      const hoverUuid = realtimeState.hoverElement.current.getAttribute("uuid");
      realtimeState.selectElements.current = makeTree(
        realtimeState.editDomRef.current,
        hoverUuid
      );
    }

    realtimeState.selectPoint.current = { x: e.clientX, y: e.clientY };
  };
  ...
  const windowMouseMove = (e) => {
    ...
    if (realtimeState.selectPoint.current) {
      const distance = Math.sqrt(
        Math.pow(Math.abs(clientX - realtimeState.selectPoint.current.x), 2) +
          Math.pow(Math.abs(clientY - realtimeState.selectPoint.current.y), 2)
      );
    ...
    if (realtimeState.selectElements.current.length > 0) {
      window.getSelection().removeAllRanges();
      setCurrentPoint({ x: clientX, y: clientY });
      setOverlayList(realtimeState.selectElements.current);
  ...
  const windowMouseUp = (e) => {
    ...
    if (realtimeState.hoverElement.current && !contextMenu && e.button === 2) {
      const { clientX, clientY } = e;
      ...
      realtimeState.contextMenuPoint.current = { x: popupX, y: popupY };
      // 한번 닫았다 열어줘서 열려있는 스크롤같은걸 닫아줌
      setContextMenuYn(false);
    }

    // Element를 옮기는 중이고, 선택된 Element가 있음
    const moveMentSideData = realtimeState.movementSideRef.current;
    if (
      realtimeState.selectElements.current.length > 0 &&
      moveMentSideData?.uuid
    ) {
      moveElementData();
    } else {
      const { clientX, clientY } = e;
      const distance = Math.sqrt(
        Math.pow(Math.abs(clientX - realtimeState.selectPoint?.x), 2) +
          Math.pow(Math.abs(clientY - realtimeState.selectPoint?.y), 2)
      );
    ...
    realtimeState.selectElements.current = [];
    realtimeState.selectPoint.current = null;
    setOverlayList([]);
    setDraggable(false);
  ...
  const modifyDomSave = async (newEditDom) => {
    const editDomList = realtimeState.editDomRef.current;
  ...
  const getClosestElement = (elements, pos, axis) => {
    if (!elements || elements.length === 0) {
      realtimeState.nearElement.current = null;
      realtimeState.hoverElement.current = null;
      return;
    }
    ...
    realtimeState.nearElement.current = nearEl;
    realtimeState.hoverElement.current = hoverEl;
  };
  ...
  const findNearElementByPointer = (x, y) => {
    ...
    if (filteredContents.length > 0) {
      ...
      if (!realtimeState.nearElement.current) {
  ...
  const decideMovementSide = (x1, y1) => {
    const targetElement =
      realtimeState.hoverElement.current || realtimeState.nearElement.current;
    ...
    const { prevData } = getSiblingsData(targetData);
    ...
    if (minDistance.position === "top") {
      if (!realtimeState.hoverElement.current) {
        //prevData가 없으면 맨위
      ...
      if (!realtimeState.hoverElement.current) {
        targetElementData.uuid = topParentSiblingsData.nextData
  ...
  const moveElementData = () => {
    const selectDatas = realtimeState.selectElements.current;
    const movementData = realtimeState.movementSideRef.current;
    ...
    if (targetData.parentId) {
      ...
      if (
        ...
        let elementSort = realtimeState.editDomRef.current.length;
        const newElement = makeNewElement({
          tagName: "multiple",
          direction: "row",
          sort: elementSort,
  ...
  const makeNewElement = ({ tagName, direction, width, sort }) => {
    ...
    const newElement = {
      ...
      sort: sort ? sort : realtimeState.editDomRef.current.length,
    };
  ...
  const changePopupYn = (e) => {
    ...
    const hoverData = getEditComponentData(
      realtimeState.hoverElement?.getAttribute("uuid")
    );
      ...
      if (hoverData.tagName === "image") {
        if (!popupYn) {
          const { bottom, left, width } =
            realtimeState.hoverElement?.getBoundingClientRect();
          let popupY = 0;
          let popupX = 0;
          ...
          realtimeState.fileData.current = {
            uuid: hoverData.uuid,
            y: popupY,
            x: popupX,
          };
  ...
  return (
    <EditorWrapper
      onContextMenu={(e) => {
        ...
        if (realtimeState.hoverElement.current) {
          setPopupUuid(
            realtimeState.hoverElement.current?.getAttribute("uuid")
          );
      ...
      >
        <OverlayContentWrapper>
          <OverlayContentContents>
            ...
            {popupYn ? (
              ...
              >
                <PopupMenu
                  ...
                  fileData={realtimeState.fileData}
                  modifyEditDom={modifyEditDom}
                />
              </div>
            ) : null}
            ...
            {contextMenuYn ? (
              <ContextMenuPopup
                pointer={realtimeState.contextMenuPoint.current}
